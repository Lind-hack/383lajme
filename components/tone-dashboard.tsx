...
export default function ToneDashboard({ summary }: { summary: ToneSummary }) {
  ...
  // Only countries the scraper has actually resolved. The rest are sitting
  // behind the per-run classification cap with nothing to say yet, and a row
  // reading "pa të dhëna" is a row spent on absence — ten of sixteen were
  // doing exactly that.
  //
  ...
  // of them contribute a structural zero reported 35% neutral on a chart that
  // was plainly ~93% grey — the lead sentence contradicting the picture under
  // it.
    ...
    <section style={{ marginBottom: "var(--space-section)" }}>
      ...
      >
        ...
        <p style={{ margin: "0 0 18px", fontSize: "15.5px", color: TONE_INK.muted, lineHeight: 1.6 }}>
          ...
          {NEUTRAL_IS_NORMAL.replace(/^Shumica e raportimit është neutrale — kjo është normale: /, "")}
        </p>

        {/* The gloss under the number is gone: the sentence above already says
            what 52 means, and saying it twice was chrome, not clarity. */}
        ...
        {/* The three-swatch legend is gone: the map's own gradient already
            runs Kritik → Pozitiv and says the same thing one line above. */}
            ...
            return (
              ...
              >
                ...
                {/* No bar. At ~93% neutral every country's stack was the same
                    ...
                    visually identical at 49 and 49. The swatch and the number
                    carry it; the full breakdown is on /toni. */}
        ...
        {/* One line where ten empty rows used to be, and the door to the full
            sixteen-country view rather than cramming it onto a news homepage. */}
