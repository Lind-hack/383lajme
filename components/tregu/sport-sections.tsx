 ...
 * as an honest locked state until its pricing algorithm exists. The animated
 * orange orbit (.tregu-sport-card::before) marks the two surfaces that are
 * actually live. Selecting a league hands the filter back to the page, which
* applies it to the floor grid and scrolls there.
 */
...
export default function SportSections({
  ...
  const f1Count = useMemo(
    () => (markets ?? []).filter((m) => (m.market_classification === "live_f1" || m.market_type === "f1_race_winner") && isOpen(m)).length,
    [markets, isOpen]
  );
  ...
  return (
    <section aria-label="Sportet në Treg" data-tour="floor-sports" style={{ marginBottom: 22 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: 16 }}>
        ...
        <article className="tregu-sport-card p-5 flex flex-col">
          <header className="flex items-center gap-2.5 mb-4">
            <Trophy size={18} strokeWidth={2} className="text-orange" aria-hidden />
        ...
        <article className="tregu-sport-card p-5 flex flex-col" data-state="locked" aria-disabled>
          <header className="flex items-center gap-2.5 mb-4">
