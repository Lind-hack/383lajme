 ...
 * stops the three drifting apart — the beam paths in particular are geometry,
 * and geometry copied three times gets edited once.
 *
 * All three competitions share the beams; only the object beneath them changes
 * — a stadium for Champions, a trophy for Europa and Conference. Colour comes
 * from the [data-competition] block in globals.css, never from here.
 *
 * (The beam classes are still named `tregu-uel-*` from when Europa was the only
 * competition using them. All three use them now; worth renaming to
 * `tregu-competition-*` at some point, separately from a feature change.)
 */
export default function CompetitionArtwork({ league }: { league?: string | null }) {
  ...
  if (!nightArt && !trophyArt) return null;
  ...
  return (
    <>
      {/* Four beams fall from above, refract off the bottom edge and carry on
          ...
          separately drawn lines. Exact translates stay exactly parallel, so the
          spacing on the outgoing leg is the spacing on the incoming one.
      ...
      {nightArt && (
        <>
          ...
          <img
            ...
            height={265}
          />
          {/* The soft field the beams cut across. Champions keeps both: the
              washes give the card its depth, the beams give it the same motion
              language as the other two. */}
