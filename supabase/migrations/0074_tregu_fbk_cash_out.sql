-- Align sport cash-outs with the NBA/FBK buy contract; preserve ledger and locking.
begin;
...
begin
  ...
  if not found
     ...
     or coalesce(v_market.live_event->>'provider', '') not in ('espn', 'fbk')
     or v_market.market_classification not in ('live_football', 'live_basketball')
     or (v_market.live_event->>'provider' = 'fbk' and (v_market.market_classification is distinct from 'live_basketball' or v_market.live_event->>'league' is distinct from 'fbk.kosovo'))
