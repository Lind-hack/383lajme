...
begin
  ...
  -- Use displayed LMSR prices, including intervening trades. Interpolate the
  -- whole probability vector; clamp-then-normalize can exceed individual caps.
  select max(value::numeric) into v_max_quantity from jsonb_each_text(v_market.outcome_quantities);
  select sum(exp((value::numeric-v_max_quantity)/v_market.b)) into v_weight_sum
    from jsonb_each_text(v_market.outcome_quantities);
  select jsonb_object_agg(key, exp((value::numeric-v_max_quantity)/v_market.b)/v_weight_sum)
    into v_current from jsonb_each_text(v_market.outcome_quantities);
  for v_key in select value->>'key' from jsonb_array_elements(v_market.sport_outcomes) value loop
    if v_current->>v_key is null then raise exception 'Missing current outcome quantity'; end if;
    v_distance := greatest(v_distance, abs((p_reference_probabilities->>v_key)::numeric-(v_current->>v_key)::numeric));
  end loop;
  v_fraction := case when v_distance=0 then 1 else least(1,v_cap/v_distance) end;
  for v_key in select value->>'key' from jsonb_array_elements(v_market.sport_outcomes) value loop
    v_new := v_new || jsonb_build_object(v_key,
      (v_current->>v_key)::numeric + v_fraction*((p_reference_probabilities->>v_key)::numeric-(v_current->>v_key)::numeric));
  end loop;
