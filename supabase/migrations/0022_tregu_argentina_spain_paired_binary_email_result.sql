-- 0022 — Return an explicit persistence result so live-email eligibility cannot be inferred from a scan.
-- Replaces 0021's void RPC; callers send mail only when this function returns true.
drop function if exists public.apply_paired_binary_sport_oracle(uuid, uuid, text, jsonb, numeric, jsonb, text, numeric, boolean, text, text, timestamptz);
create function public.apply_paired_binary_sport_oracle(
...
begin
  ...
  if v_spain.live_score_state->>'key' = p_state->>'key' and v_argentina.live_score_state->>'key' = p_state->>'key' then return false; end if;
