-- Official sport updates may make structural moves; ordinary news caps remain unchanged.
...
alter table public.market_snapshots add constraint market_snapshots_oracle_cap_check
  check (oracle_cap >= 0 and oracle_cap <= case when oracle_kind in ('sport_oracle', 'f1_oracle') then 1.0 else 0.10 end);
