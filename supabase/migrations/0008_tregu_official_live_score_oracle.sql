-- 0008 — Official live-score oracle for active sporting markets.
-- This migration is submitted for review only. It is not applied by this change.

alter table public.markets
  add column if not exists live_event jsonb,
  add column if not exists live_score_state jsonb;
...
alter table public.market_snapshots add constraint market_snapshots_oracle_cap_check
  check (oracle_cap is null or (oracle_cap > 0 and oracle_cap <= 0.10));
...
begin
  ...
  if p_close_market and p_state->>'status' !~ '(FINAL|POST|CANCELED)' then
