...
test("range controls expose the complete legacy key set and disclose the zoom scale", () => {
  ...
  assert.match(exact, /angularRecordedPath\(displayPoints, model\.x, model\.y\)/);
  assert.match(exact, /recordedRangeDisplaySeries\(selected\.series, selected\.start, selected\.end\)/);
  assert.match(exact, /timelineIndex/);
...
test("successful mobile buys prime and play a bounded Web Audio confirmation", () => {
  assert.match(mobileTrade, /primeTradeSuccessSound\(\)/);
  assert.match(mobileTrade, /playTradeSuccessSound\(\)/);
  assert.match(tradeSound, /context\.createOscillator\(\)/);
  assert.match(tradeSound, /\[523\.25, 659\.25, 783\.99\]/);
