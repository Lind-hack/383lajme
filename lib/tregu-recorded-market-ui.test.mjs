...
const navBalance = read("components/nav-balance.tsx");
const coinToast = read("components/tregu/coin-toast.tsx");
...
test("range controls expose the complete legacy key set and disclose the zoom scale", () => {
  ...
  assert.match(exact, /onPointerUp=\{finishTouchInspection\}/);
  assert.match(exact, /onPointerCancel=\{finishTouchInspection\}/);
  assert.match(exact, /Math\.abs\(model\.x\(candidate\) - clampedX\)/);
  assert.match(exact, /Prek ose rrëshqit për detaje/);
  assert.match(exact, /tregu-exact-chart-inspector/);
  assert.match(styles, /left: clamp\(96px, var\(--inspection-x\), calc\(100% - 96px\)\)/);
...
test("earned-coin motion carries the exact reward instead of the default coin numeral", () => {
  assert.match(hubPage, /amount: earned/);
  assert.match(hubPage, /rewardAmount=\{rewardAmount\}/);
  assert.match(mobileBar, /numeral=\{`\+\$\{fmtNum\(coin\.amount\)\}`\}/);
  assert.match(mobileBar, /coinSpin && rewardAmount \? `\+\$\{fmtNum\(rewardAmount\)\}` : "383"/);
  assert.match(navBalance, /amount: earned/);
  assert.match(navBalance, /numeral=\{`\+\$\{fmtNum\(c\.amount\)\}`\}/);
  assert.match(navBalance, /spinning && activeRewardAmount \? `\+\$\{fmtNum\(activeRewardAmount\)\}` : "383"/);
  assert.match(coinToast, /\+\{fmtNum\(toast\.amount\)\} 383C në bilanc/);
});
...
test("the mobile first viewport keeps title, odds, ranges, chart and sharing compact", () => {
  ...
  assert.match(styles, /\.tregu-detail-chart-shell \.tregu-exact-chart-plot \{[\s\S]*?height: 268px !important/);
  ...
  assert.match(shareCard, /import \{ ImageResponse \} from "next\/og"/);
  assert.match(shareCard, /return new ImageResponse\(/);
});
...
test("successful mobile buys prime and play bounded sport-context Web Audio feedback", () => {
  ...
  assert.match(tradeSound, /cache: "reload"/);
  assert.doesNotMatch(tradeSound, /if \(profile === "football"\) playFootballCue/);
...
test("purchase feedback uses team material finishes and separate mobile and desktop motion", () => {
  // Football and newsroom markets take no finish: each variant mixes its own
  // dark into the surface (carbon 48% #111317, parquet 32% brown), and a
  // character-code hash of the club name used to pick one at random — a second
  // darkening on top of the wash, unrelated to the club. The kit colour carries
  // the identity now. F1 and basketball keep their materials.
  assert.match(detail, /ferrari\|red bull[\s\S]*?return "speed"/);
  assert.match(detail, /sportTheme === "basketball"\) return "parquet"/);
  assert.match(detail, /sportTheme === "f1"\) return "metallic"/);
  assert.doesNotMatch(detail, /finishes\[hash % finishes\.length\]/);
