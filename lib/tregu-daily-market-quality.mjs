import { NEWS_CONTRACT_VERSION, validateNewsProposition } from "./tregu-news-contract.mjs";
...
function scoreCandidate(candidate, { sourceCount, familyCount, archetype, audienceTier }) {
  ...
  if (candidate.contract_version === NEWS_CONTRACT_VERSION
    ? hours >= 30 * 24 && hours <= 90 * 24
    : hours != null && hours >= 12 && hours <= 72) score += 1;
  return score;
}
...
export function evaluateDailyMarketCandidate(candidate, options = {}) {
  ...
  if (!eventContract && !deadlineFromQuestion(question)) reasons.push("missing_concrete_deadline");
  if (eventContract) {
    const violation = validateNewsProposition(candidate);
  ...
  if (/20\d{2}|(?:janar|shkurt|mars|prill|maj|qershor|korrik|gusht|shtator|tetor|nentor|dhjetor)/.test(topicKey)) reasons.push("topic_key_must_not_encode_deadline");
  ...
  if (hours == null) reasons.push("closes_in_hours_required");
    ...
    const maxHours = eventContract ? 90 * 24 : archetype === "scheduled_decision" ? MAX_SCHEDULED_DECISION_HOURS : MAX_BREAKING_HOURS;
    if (hours < (eventContract ? 30 * 24 : 8) || hours > maxHours) reasons.push("deadline_outside_trading_window");
    const closesAt = new Date(now.getTime() + hours * 3_600_000);
    if ((!eventContract || deadlineFromQuestion(question)) && !deadlineMatchesQuestion(question, closesAt)) reasons.push("title_deadline_does_not_match_close");
  ...
  if (eventContract && families.size < 2) reasons.push("independent_publishers_required");
