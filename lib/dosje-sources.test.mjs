...
import { relevanceGroupsForQuery, sourceMatchesProfile, fetchSource, publisherOf, WAYBACK_AVAILABILITY, timestampForQuery, isFetchableCitationUrl } from "./dosje-sources.mjs";
...
test("the seed's placeholder citation is not treated as a link that can be fetched", () => {
  // Two of these are live in dosje_citations. The re-verification job fetched
  // them nightly and mailed the newsroom that a source had died.
  assert.equal(isFetchableCitationUrl("https://example.invalid/unverified"), false);
});

test("reserved hosts and non-http schemes are refused", () => {
  for (const url of [
    "https://foo.example/a",
    "https://a.test/b",
    "http://localhost:3000/a",
    "http://LOCALHOST/a",
    "ftp://balkaninsight.com/a",
    "javascript:alert(1)",
    "",
    "not a url",
  ]) {
    assert.equal(isFetchableCitationUrl(url), false, url);
  }
});

test("a real citation url is still fetchable", () => {
  for (const url of [
    "https://news.bbc.co.uk/1/hi/world/europe/7249034.stm",
    "https://balkaninsight.com/2020/09/04/kosovo-and-serbia-sign-deal/",
    "http://www.rferl.org/a/kosovo-police-officer-killed",
  ]) {
    assert.equal(isFetchableCitationUrl(url), true, url);
  }
});

test("a subdomain of a reserved name is refused with it", () => {
  assert.equal(isFetchableCitationUrl("https://archive.example.invalid/x"), false);
});
