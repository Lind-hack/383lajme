...
import {
  ...
  sportOutcomePrices,
  isDuplicateSportOracle,
} from "./tregu-sport-market.mjs";
test("only an atomic duplicate sports state is treated as an unchanged replay", () => {
  assert.equal(isDuplicateSportOracle({ code: "23505", message: 'duplicate key violates "sport_oracle_events_market_id_state_key_key"' }), true);
  assert.equal(isDuplicateSportOracle({ code: "23505", message: 'another unique constraint' }), false);
  assert.equal(isDuplicateSportOracle(null), false);
});
...
test("official score, clock, final state, and verified lineup/news inputs drive bounded transparent references", () => {
  ...
  assert.equal(signal.snapshot.oracle_cap, 1);
...
test("a stoppage-time equalizer immediately makes the draw overwhelmingly likely", () => {
  const live = normalizeEspnSummary(market.live_event, summary());
  const late = { ...live, detail: "90+4'", competitors: live.competitors.map(t => ({ ...t, score: 1 })) };
  const [signal] = buildSportMarketPlan({ markets: [market], events: [late] });
  assert.ok(signal.snapshot.reference_probabilities.draw > .9);
  assert.equal(signal.snapshot.oracle_cap, 1);
  assert.ok(Object.values(signal.snapshot.reference_probabilities).every(p => Number.isFinite(p) && p > 0));
});
