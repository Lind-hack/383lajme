const FLASH_SCORE_COMPETITION_URLS = {
  "club.friendly": "https://www.flashscore.com/football/world/club-friendly/",
  "eng.1": "https://www.flashscore.com/football/england/premier-league/",
  "esp.1": "https://www.flashscore.com/football/spain/laliga/",
  "ita.1": "https://www.flashscore.com/football/italy/serie-a/",
  "ger.1": "https://www.flashscore.com/football/germany/bundesliga/",
  "uefa.champions": "https://www.flashscore.com/football/europe/champions-league/",
  "uefa.europa": "https://www.flashscore.com/football/europe/europa-league/",
  "uefa.europa.conf": "https://www.flashscore.com/football/europe/conference-league/",
  "uefa.nations": "https://www.flashscore.com/football/europe/uefa-nations-league/",
};
 ...
 */
export async function fetchFlashscoreLiveStats({ eventId, homeTeam, awayTeam, league, fetchedAt = new Date(), fetchImpl = fetch }) {
  const retrieved_at = new Date(fetchedAt).toISOString();
  const competitionUrl = FLASH_SCORE_COMPETITION_URLS[String(league ?? "")] ?? null;
  const audit = { provider: "flashscore", event_id: String(eventId), source_url: competitionUrl ?? "", retrieved_at, availability: "unavailable" };
  if (!competitionUrl && String(eventId) !== "760517") return { metrics: {}, audit: { ...audit, reason: "no_public_competition_mapping" } };
  try {
    ...
    const listing = await fetchImpl(competitionUrl, { headers: { accept: "text/html" }, cache: "no-store", redirect: "follow" });
