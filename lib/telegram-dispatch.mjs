...
export function renderCaption(article, { captionLimit = CAPTION_LIMIT } = {}) {
  ...
  // Written out rather than hidden behind anchor text, and short. These posts
  // get copied into WhatsApp, which cannot hyperlink text and cannot link back
  // to a channel: copying an <a> leaves the URL behind, and pasting the
  // canonical one drops ninety characters of slug above the preview card.
  const linkPart = `Lexo më shumë: ${escapeHtml(shortUrl(slug))}`;
 ...
 */
/**
 * The tappable button under a channel post.
 *
 * It duplicates the link already in the caption on purpose. The button is the
 * affordance readers use inside Telegram; the caption line is what survives
 * being copied into WhatsApp, which has no buttons and cannot label a link.
 * Drop either one and one of those two audiences loses its way in.
 */
export function replyMarkup(article) {
  return {
    inline_keyboard: [[
      {
        text: "Lexo më shumë në 383 →",
        url: shortUrl(String(article.slug ?? "")),
      },
    ]],
  };
}
