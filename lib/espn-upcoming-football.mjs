...
export function buildUpcomingFootballTemplate(fixture) {
  ...
  const equalProbability = 1 / outcomeKeys.length;
  const probabilities = Object.fromEntries(
    outcomeKeys.map((key) => [key, equalProbability])
  );
  const quantities = Object.fromEntries(outcomeKeys.map((key) => [key, 0]));
  ...
  return {
    ...
    description: `Treg live për ${fixture.league_label}; formati i fazës dhe rezultati zyrtar verifikohen nga ESPN.`,
    category: "sport",
    status: "open",
    ...
    resolution_source: "ESPN official match summary",
    live_event: {
      provider: "espn",
