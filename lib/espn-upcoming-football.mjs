...
const teamFrom = (competitor) => ({
  ...
  // The club's second colour. ESPN has always published it; nothing captured it,
  // so a two-colour club (Barcelona's garnet and blue) only ever reached the UI
  // as one flat crimson.
  alternate_color: competitor?.team?.alternateColor ?? null,
...
export function buildUpcomingFootballTemplate(fixture) {
  ...
  return {
    ...
    closes_at: new Date(Date.parse(fixture.kickoff) + 6 * 3600000).toISOString(),
