...
/**
 * How much where you start decides whether you win.
 *
 * The old model scored the grid with rankScore at weight 0.55 - a decay of
 * 0.041 per place, so twentieth on the grid was worth 46% of pole. That is not
 * how a Grand Prix works, and it produced the failure this replaces: a driver
 * leading the championship kept a 15% chance of winning Monza while serving a
 * penalty that put him at the back, because the championship term outweighed
 * the grid term by nearly twenty to one.
 *
 * 0.26 per place puts pole about 135 times a back-row start, which is the right
 * order for the modern era: pole converts to a win roughly 40% of the time, P10
 * a couple of per cent, and a win from twentieth is a race people still talk
 * about years later. It stays exponential rather than going to zero, because
 * those races do happen.
 *
 * Neutral - not penal - when the grid is unknown: before qualifying every
 * driver is equally unplaced, and normalisation cancels a constant.
 */
function gridScore(position) {
  const rank = finite(position);
  if (rank === null || rank < 1) return null;
  return Math.exp(-0.26 * (rank - 1));
}
...
export function buildF1RaceWinnerOpeningModel({
  ...
  for (const driver of drivers) {
    ...
    // The grid a driver actually starts from: the published grid when it is
    // known, otherwise qualifying moved back by whatever penalty has been
    // verified since. A penalty is applied here rather than left to the news
    // repricer so the opening price is right the first time it is published.
    const penalty = penalties?.[key] ?? null;
    const penaltyPlaces = finite(penalty?.grid_penalty_places) ?? 0;
    const publishedGrid = finite(grid?.[key] ?? grid?.[String(driver?.driver_number ?? "")]);
    const startingGrid = publishedGrid !== null
      ? publishedGrid
      : qualifyingPosition !== null
        ? Math.min(drivers.length, qualifyingPosition + penaltyPlaces)
        : null;
    const gridFactor = startingGrid === null ? 1 : gridScore(startingGrid);
    // Practice pace, reliability, what the driver said on the radio. Real
    // information, and the least verifiable kind on the page, so it is bounded
    // at a fifth either way: enough to separate two cars starting side by side,
    // never enough to outrank where they are starting from. These drivers are
    // never named as structural, so the move it produces is still clamped by
    // the ordinary five-point cap on its way into the book.
    const signalDelta = Math.max(-1, Math.min(1, finite(signals?.[key]?.delta) ?? 0));
    const signalFactor = clamp(1 + 0.22 * signalDelta, 0.78, 1.22);
    ...
    // A driver who is out is out: withdrawn, disqualified or not starting is a
    // fact about the race, not a low opinion of their chances, and a market
    // that still quotes them is simply wrong.
    const isOut = penalty?.status === "out";
    const value = isOut ? 0 : Math.exp(
      0.95 * Math.log(championshipFactor ?? 0.35) +
      0.80 * Math.log(teamFactor) +
      0.85 * Math.log(recentFactor) +
      0.65 * Math.log(circuitFactor) +
      1.00 * Math.log(gridFactor) +
      1.00 * Math.log(signalFactor) +
      0.20 * Math.log(qualifyingFactor) +
    ...
    inputs[key] = {
      ...
      grid_penalty_places: penaltyPlaces || null,
      penalty_reason: penalty?.reason ?? null,
      penalty_source: penalty?.source ?? null,
      not_starting: isOut || null,
      signal_delta: signalDelta || null,
      signal_reason: signals?.[key]?.reason ?? null,
      simulator_probability: simulatorProbability,
      factors: { championshipFactor, teamFactor, recentFactor, circuitFactor, gridFactor, signalFactor, qualifyingFactor, simulatorFactor },
    };
  }

  const probabilities = normalize(raw, drivers.map((driver) => clean(driver?.key ?? driver?.driver_code).toUpperCase()));
  return {
    model_version: "f1-opening-v4",
    probabilities,
    inputs,
    availability,
