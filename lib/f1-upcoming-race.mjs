...
export async function fetchUpcomingOpenF1Race({ now = new Date(), leadDays = 3, fetchImpl = fetch } = {}) {
  ...
  return race ? { ...race, ...await fetchF1RaceDistance(race, { fetchImpl }), event_id: raceEventId(race), source_url: `${OPENF1}/sessions?year=${race.year}&session_name=Race` } : null;
}
...
export async function fetchF1OpeningFactors(options = {}) {
  try {
    return await fetchOpenF1OpeningFactors(options);
  } catch (error) {
