...
test("admin drafts can be classified as General / News, Live Football, or Live F1", () => {
  ...
  const basketballMigration = source("supabase/migrations/0029_tregu_live_basketball_classification.sql");
  ...
  assert.match(client, /<option value="live_football">Football — score, cards, time<\/option>/);
  assert.match(client, /<option value="live_f1">F1 — official live timing<\/option>/);
  assert.match(client, /<option value="live_basketball">Basketball — NBA \/ FBK Superliga<\/option>/);
  ...
  assert.match(basketballMigration, /market_classification in \('general_news', 'live_football', 'live_basketball', 'live_f1'\)/);
