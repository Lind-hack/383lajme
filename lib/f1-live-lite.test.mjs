...
test("does not emit an oracle write when the official leaderboard state is unchanged", () => {
  const leaderboard = parseF1LiveLiteLeaderboard(renderedLeaderboard);
  const plan = buildF1MarketPlan({
    markets: [{ id: "piastri", slug: "f1-belgjika-spa-fiton-oscar-piastri", live_score_state: { key: JSON.stringify({ position: 1, driver: "Oscar Piastri", gap: "1:23:45.111", pits: 2 }) }, q_yes: 0, q_no: 0, b: 100, status: "open" }],
    leaderboard,
  });
  assert.deepEqual(plan, []);
});

test("parses a rendered columnar leaderboard when every official cell is on its own line", () => {
  const leaderboard = parseF1LiveLiteLeaderboard(`
    POS
    DRIVER
    GAP
    PITS
  ...
  `);
  assert.deepEqual(leaderboard.rows, [
    { position: 1, driver: "Oscar Piastri", gap: "LEADER", pits: 2 },
    { position: 2, driver: "Lando Norris", gap: "+2.481", pits: 2 },
    { position: 3, driver: "Max Verstappen", gap: "+1 LAP", pits: 1 },
  ]);
});

test("only persists a driver market when that driver's official position, gap, or pit state changes", () => {
  const leaderboard = parseF1LiveLiteLeaderboard(renderedLeaderboard);
  const piastri = leaderboard.rows[0];
  const unchangedPiastriKey = JSON.stringify({ position: piastri.position, driver: piastri.driver, gap: piastri.gap, pits: piastri.pits });
  const plan = buildF1MarketPlan({
    markets: [
      { id: "piastri", slug: "f1-belgjika-spa-fiton-piastri", status: "open", live_score_state: { key: unchangedPiastriKey } },
      { id: "norris", slug: "f1-belgjika-spa-fiton-norris", status: "open" },
    ],
    leaderboard,
  });
  assert.deepEqual(plan.map((item) => item.market.id), ["norris"]);
  assert.match(plan[0].reasoning, /gap \+2\.481.*2 pit/i);
});
