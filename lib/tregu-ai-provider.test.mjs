...
import { parseJSON } from "./groq.ts";

const json = (body, status = 200) => new Response(JSON.stringify(body), { status, headers: { "content-type": "application/json" } });

test("news scoring sends bounded source bodies and a sufficient JSON completion budget", () => {
  const source = readFileSync(new URL("./tregu.ts", import.meta.url), "utf8");
  assert.match(source, /String\(a\.body \?\? ""\)\.slice\(0, bodyBudget\)/);
  assert.match(source, /marketAiChat\(system, user, \{ json: true, maxTokens: 900 \}\)/);
  assert.match(source, /filtered evidence/);
  const providerSource = readFileSync(new URL("./tregu-ai-provider.mjs", import.meta.url), "utf8");
  assert.match(providerSource, /gemini-3\.6-flash/);
});

test("JSON parser preserves model reasoning with raw line controls inside strings", () => {
  const raw = '{"reasoning":"line one' + String.fromCharCode(10) + 'line two"}';
  assert.deepEqual(parseJSON(raw), { reasoning: "line one\nline two" });
});
...
test("Groq primary uses a model currently present in the live Groq catalog", async () => {
  let requestBody = null;
  await marketAiChat("system", "user", { json: true }, {
    env: { GROQ_API_KEY: "groq", GOOGLE_API_KEY: "google-one" },
    fetchImpl: async (_url, init) => {
      requestBody = JSON.parse(String(init?.body ?? "{}"));
      return json({ choices: [{ message: { content: '{"probability":0.5}' } }] });
    },
  });
