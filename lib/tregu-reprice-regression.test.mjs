...
import { buildRepricePlan, newsDeadlineAction, newsDeadlineDecayCap, repriceMarketSkipReason } from "./tregu-automation.mjs";
...
test("deadline actions settle after expiry and enter the measured final 72-hour countdown", () => {
  ...
  assert.equal(newsDeadlineDecayCap(48), 0.01);
  assert.equal(newsDeadlineDecayCap(24), 0.02);
  assert.equal(newsDeadlineDecayCap(6), 0.03);
  assert.equal(newsDeadlineDecayCap(2), 0.03);
  const automation = readFileSync(new URL("./tregu-automation-server.ts", import.meta.url), "utf8");
