...
import { buildRepricePlan, newsDeadlineAction, newsDeadlineDecayCap, NEWS_DEADLINE_DECAY_INTERVAL_MS, repriceMarketSkipReason } from "./tregu-automation.mjs";
...
test("deadline actions settle after expiry and enter the measured final 72-hour countdown", () => {
  const now = new Date("2026-07-15T20:00:00.000Z");
  ...
  assert.equal(newsDeadlineAction({ ...base, closes_at: "2026-07-18T08:00:00.000Z" }, now), "decay");
  assert.equal(newsDeadlineAction({ ...base, closes_at: "2026-07-19T20:01:00.000Z" }, now), null);
  assert.equal(newsDeadlineAction({ ...base, category: "sport", closes_at: "2026-07-15T19:59:00.000Z" }, now), null);
  assert.equal(newsDeadlineDecayCap(73), null);
  assert.equal(newsDeadlineDecayCap(48), 0.001);
  assert.equal(newsDeadlineDecayCap(24), 0.002);
  assert.equal(newsDeadlineDecayCap(6), 0.004);
  assert.equal(newsDeadlineDecayCap(2), 0.004);
...
test("deadline RPCs are guarded to open binary general-news markets and report only a persisted change", () => {
  ...
  const migration = readFileSync(new URL("../supabase/migrations/0060_tregu_deadline_decay_window.sql", import.meta.url), "utf8");
