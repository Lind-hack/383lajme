...
/**
 * The bar itself was never the problem; what it was measured against was.
 *
 * The subjects scan reads 200 recent articles, and on the live archive about
 * one in a hundred matches any dossier subject -- correctly, since the near
 * misses are Serbia's own elections, Montenegro's EU path, North Macedonia,
 * Albania and Libya. Two matches per scan against a bar of three articles over
 * two days meant no subject could ever qualify from a single run, which is why
 * the research job answered no_candidate every night.
 */
test("three articles over two days is a standing subject", () => {
  assert.equal(isStandingSubject([
    { articleSlug: "a", publishedAt: "2026-09-01T10:00:00Z" },
    { articleSlug: "b", publishedAt: "2026-09-01T18:00:00Z" },
    { articleSlug: "c", publishedAt: "2026-09-03T09:00:00Z" },
  ]), true);
});

test("a busy single day is one story, not a file", () => {
  assert.equal(isStandingSubject([
    { articleSlug: "a", publishedAt: "2026-09-01T08:00:00Z" },
    { articleSlug: "b", publishedAt: "2026-09-01T12:00:00Z" },
    { articleSlug: "c", publishedAt: "2026-09-01T20:00:00Z" },
  ]), false, "same day three times is still one news cycle");
});

test("two articles never qualify, however far apart", () => {
  assert.equal(isStandingSubject([
    { articleSlug: "a", publishedAt: "2026-01-01T08:00:00Z" },
    { articleSlug: "b", publishedAt: "2026-09-01T08:00:00Z" },
  ]), false);
});

test("the same article re-scanned cannot count twice toward the bar", () => {
  // The selector dedupes by article before calling this, so a repeat arrives
  // collapsed. Guarded here because a regression there would silently promote
  // one article seen on three runs into a standing subject.
  const dedup = new Map();
  for (const r of [
    { articleSlug: "a", publishedAt: "2026-09-01T08:00:00Z" },
    { articleSlug: "a", publishedAt: "2026-09-02T08:00:00Z" },
    { articleSlug: "a", publishedAt: "2026-09-03T08:00:00Z" },
  ]) dedup.set(r.articleSlug, r);
  assert.equal(isStandingSubject([...dedup.values()]), false);
});

test("a row with no publish date is ignored rather than counted as today", () => {
  assert.equal(isStandingSubject([
    { articleSlug: "a", publishedAt: "2026-09-01T08:00:00Z" },
    { articleSlug: "b", publishedAt: "2026-09-02T08:00:00Z" },
    { articleSlug: "c", publishedAt: null },
  ]), false, "two dated articles is still short of the bar");
});
