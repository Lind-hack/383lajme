...
test("pre-match endpoint, database lock, health card, and runner remain server-side and auditable", () => {
  ...
  // The health card no longer prints the provider chain ("Groq → Google"); naming
  // AI vendors in an operator-facing panel was cosmetic. What must stay true is
  // that refresh health for both the verified-news and official-sport paths is
  // surfaced, and that the panel still reports how many markets were scanned.
  assert.match(adminPanel, /<HealthSource\s+title="Lajme të verifikuara \(2 min\)"\s+source=\{live\}/);
  assert.match(adminPanel, /<HealthSource\s+title="Procesori zyrtar sportiv \(2 min\)"\s+source=\{sports\}/);
  assert.match(adminPanel, /last_successful_refresh/);
