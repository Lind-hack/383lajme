 ...
 *
 * The page hands timelineFor the fifty most recent articles. A reader on an
 * older story therefore found their own piece missing from its own file, and
 * the account ran on past it to end on a later report by someone else.
 */

test("the article being read is in its own file even when the fetch missed it", () => {
  const current = article({
    slug: "vjeter",
    title: "Bisedimet në Bruksel për normalizim, raundi i parë",
    publishedAt: "2026-01-05",
  });
  // The fetch returned only newer coverage; the reader's own story is not in it.
  const fetched = [
    article({ slug: "reja", title: "Bisedimet në Bruksel për normalizim, raundi i dytë", publishedAt: "2026-08-20" }),
  ];

  const tl = timelineFor("dialogu-kosove-serbi", fetched, "vjeter", current);
  const slugs = tl.filter((e) => e.kind === "article").map((e) => e.slug);

  assert.ok(slugs.includes("vjeter"), "the story being read belongs in its own file");
  assert.equal(tl.filter((e) => e.isCurrent).length, 1, "and is the one marked current");
});

test("the account ends on the story being read, not on a later one", () => {
  const current = article({
    slug: "kete",
    title: "Bisedimet në Bruksel për normalizim, raundi i dytë",
    publishedAt: "2026-05-10",
  });
  const arts = [
    article({ slug: "para", title: "Bisedimet në Bruksel për normalizim, raundi i parë", publishedAt: "2026-01-05" }),
    current,
    article({ slug: "pas", title: "Bisedimet në Bruksel për normalizim, raundi i tretë", publishedAt: "2026-08-20" }),
  ];

  const slugs = timelineFor("dialogu-kosove-serbi", arts, "kete", current)
    .filter((e) => e.kind === "article")
    .map((e) => e.slug);

  assert.deepEqual(slugs, ["para", "kete"], "everything published after the article is cut");
  assert.equal(slugs.at(-1), "kete", "the file closes on the page it is attached to");
});

test("without a current article the timeline is unchanged", () => {
  // The archive page at /dosje/[slug] has no article to end on and must keep
  ...
  const slugs = timelineFor("dialogu-kosove-serbi", arts)
    .filter((e) => e.kind === "article")
    .map((e) => e.slug);

  assert.deepEqual(slugs, ["para", "pas"]);
});

test("a current article with no usable date truncates nothing", () => {
  const current = article({
    slug: "pa-date",
    title: "Bisedimet në Bruksel për normalizim, pa datë",
  ...
  const slugs = timelineFor("dialogu-kosove-serbi", arts, "pa-date", current)
    .filter((e) => e.kind === "article")
    .map((e) => e.slug);

  assert.ok(slugs.includes("para"), "a missing date must not empty the file");
  assert.ok(slugs.includes("pa-date"));
});

test("Kosovo in the same story as Serbia's president is a dialogue story", () => {
  // The live archive's one qualifying article matched nothing: the Kosovo
  // groups wanted the word "Serbia" where the story said "presidenti serb",
  // and the groups naming Vučić all required Kurti beside him.
  const t = topicForArticle(article({
    title: "Vuçiq rikthen retorikën për Kosovën dhe pretendon pa prova se Thaçi do të lirohet",
    excerpt: "Presidenti serb përdori një tubim në Uzhicë për mesazhe të ashpra ndaj Prishtinës.",
    category: "Politikë",
  }));
  assert.equal(t?.slug, "dialogu-kosove-serbi");
});

test("Serbia's president without Kosovo is Serbian politics, not the file", () => {
  // The pairing must not turn every mention of Vučić into Kosovo coverage.
  assert.equal(
    topicForArticle(article({
      title: "Serbia mund të shkojë në zgjedhje të parakohshme në tetor",
      excerpt: "Vucic tha se vendimi do të merret pas konsultimeve brenda koalicionit qeverisës.",
      category: "Botë",
    })),
    null
  );
