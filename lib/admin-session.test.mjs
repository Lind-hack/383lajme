...
/**
 * The three side panels used a per-panel cookie whose value was the literal
 * string "1". httpOnly keeps page scripts out of a cookie; it does not stop the
 * person holding the browser from setting one in devtools or curl, and "1"
 * leaves nothing to guess. Sending `Cookie: poll_admin_auth=1` to production
 * returned the whole panel, publish controls included.
 */
test("the old panel cookie value cannot authenticate anything", () => {
  for (const forged of ["1", "true", "yes", "0", "admin"]) {
    assert.equal(verifyAdminSession(forged, SECRET), false, `accepted ${forged}`);
  }
});

test("one session is valid across every panel, since they share a secret", () => {
  // The panels now mint and verify the same token rather than each keeping a
  // private door. A session minted for one is therefore good for all of them,
  // and revoked for all of them at once when the secret rotates.
  const token = mintAdminSession(SECRET);
  assert.equal(verifyAdminSession(token, SECRET), true);
  assert.equal(verifyAdminSession(token, "rotated-secret"), false);
});
