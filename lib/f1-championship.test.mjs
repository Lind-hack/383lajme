...
const standings = [
  { driver_number: 1, points_current: 240, points_start: 222, position_current: 1 },
  { driver_number: 2, points_current: 190, points_start: 175, position_current: 2 },
  { driver_number: 3, points_current: 130, points_start: 120, position_current: 3 },
];
...
test("automation and settlement keep the title market active until mathematically decided", () => {
  ...
  const migration = readFileSync(new URL("../supabase/migrations/0061_tregu_f1_championship_settlement.sql", import.meta.url), "utf8");
  assert.match(server, /runF1ChampionshipAutomation/);
  assert.match(server, /championship\.model\.decided/);
  assert.match(migration, /event_kind' <> 'championship'/);
  assert.match(migration, /championship'->>'decided'/);
  assert.match(migration, /status = 'resolved'/);
  assert.match(migration, /openf1_championship_decided/);
});
