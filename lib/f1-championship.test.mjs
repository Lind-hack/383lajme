...
test("a 59-point lead with eleven races left is strong but not treated as certain", () => {
  const manyRaces = Array.from({ length: 11 }, (_, index) => ({ sprint: false, meeting: { meeting_key: 300 + index, circuit_short_name: index % 2 ? "Monza" : "Madring" } }));
  const model = buildChampionshipModel(context({
    standings: [
      { ...standings[0], points_current: 242 },
      { ...standings[1], points_current: 183 },
      { ...standings[2], points_current: 183 },
    ],
    recentRaces: [],
    trackBaselines: [],
    teamStandings: [
      { team_name: "Orange", points_current: 425 },
      { team_name: "Ink", points_current: 425 },
    ],
    remainingEvents: manyRaces,
    stateKey: "59-point-lead-eleven-races",
  }), { simulations: 4000 });
  assert.ok(model.probabilities.AAA > 0.45);
  assert.ok(model.probabilities.AAA < 0.9);
});
...
test("the persistent championship template uses the proven F1 winner book", () => {
  ...
  assert.match(template.live_score_state.key, /championship-monte-carlo-v3$/);
...
test("automation and settlement keep the title market active until mathematically decided", () => {
  ...
  const featured = readFileSync(new URL("../components/tregu/featured-carousel.tsx", import.meta.url), "utf8");
  ...
  assert.match(server, /f1-championship:v3/);
  assert.match(server, /template\.live_score_state\.key/);
  assert.match(server, /15 \* 60_000/);
  assert.match(server, /Math\.floor\(now\.getTime\(\) \/ \(15 \* 60_000\)\)/);
  assert.match(server, /sport_outcomes: template\.sport_outcomes/);
  assert.match(server, /Cannot remove an F1 outcome with an open position/);
  assert.match(server, /preserving existing driver trade deltas/);
  ...
  assert.match(featured, /market\.eventKind === "championship"/);
  assert.match(featured, /championshipDrivers/);
  assert.match(featured, /Hap tregun e titullit/);
});
