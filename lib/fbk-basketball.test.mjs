...
import { fbkKickoff, findFbkLeagueUrl, parseFbkFixtures, fetchFbkFixtures, buildFbkMarkets, parseFbkStandings, fbkOpeningModel, normalizeFbkFixture, fbkOfficialFinal, normalizeFbkTeam } from "./fbk-basketball.mjs";
...
test("FBK opening model responds symmetrically to observed team strength", () => {
  ...
  // Strength alone is symmetric; home court is a separate, explicit edge.
  const neutral = fbkOpeningModel(event, standings, { homeAdvantage: 0 });
  const reverse = fbkOpeningModel({ ...event, home_team: "B", away_team: "A" }, standings, { homeAdvantage: 0 });
  assert.ok(Math.abs(reverse.probabilities.home - neutral.probabilities.away) < 1e-12);
  const equal = [{ team: "A", played: 4, wins: 2, losses: 2 }, { team: "B", played: 4, wins: 2, losses: 2 }];
  assert.ok(fbkOpeningModel(event, equal).probabilities.home > .5);
  assert.deepEqual(fbkOpeningModel(event, []).probabilities, { home: .5, away: .5 });
  assert.match(model.method, /not bookmaker odds/);
});

test("FBK never prices or settles a score published before the game can be over", () => {
  ...
  // Tip-off is 17:00Z; a score seen before 19:00Z may still be moving.
  for (const now of [new Date("2026-09-08T00:00:00Z"), new Date("2026-09-19T18:30:00Z")]) {
...
test("FBK federation result settles the market once the game is over, forfeits included", () => {
  const [fixtureEvent] = parseFbkFixtures(fixture(), url);
  const [market] = buildFbkMarkets([fixtureEvent], { now: new Date("2026-09-17T00:00:00Z") });
  const now = new Date("2026-09-20T00:00:00Z");
  assert.deepEqual(fbkOfficialFinal({ ...fixtureEvent, home_score: 90, away_score: 80 }, now), { home: 90, away: 80 });
  assert.equal(fbkOfficialFinal({ ...fixtureEvent, home_score: 80, away_score: 80 }, now), null);
  assert.equal(fbkOfficialFinal({ ...fixtureEvent, home_score: 90, away_score: null }, now), null);
  for (const [home, away, winner] of [[101, 106, "away"], [96, 60, "home"], [0, 20, "away"]]) {
    const event = normalizeFbkFixture({ ...fixtureEvent, home_score: home, away_score: away }, now);
    assert.equal(event.status, "STATUS_FINAL");
    const [signal] = buildSportMarketPlan({ markets: [market], events: [event], now });
    assert.equal(signal.close_market, true);
    assert.equal(signal.verified_outcome, winner);
  }
  assert.equal(normalizeFbkFixture({ ...fixtureEvent, home_score: 90, away_score: 80 }, now, { allowFinal: false }).has_official_score, false);
});

test("FBK opening model reads point margins, shrinks early records and matches drifting names", () => {
  const event = { home_team: "Sigal Prishtina", away_team: "Peja", source_url: url };
  const standings = [{ team: "KB Prishtina", played: 10, wins: 8, losses: 2, scored: 880, conceded: 780 }, { team: "Peja", played: 10, wins: 3, losses: 7, scored: 790, conceded: 850 }];
  assert.equal(normalizeFbkTeam("Sigal Prishtina"), normalizeFbkTeam("KB Prishtina"));
  const model = fbkOpeningModel(event, standings);
  assert.match(model.method, /point margin/);
  // +10 vs -6 a game, shrunk by 10/16, plus 3 of home court: about 13 points.
  assert.ok(Math.abs(model.inputs.expected_margin - 13) < 0.1);
  assert.ok(model.probabilities.home > .8 && model.probabilities.home < .9);
  // One game in, even a 20-point win barely moves the line.
  const early = fbkOpeningModel(event, [{ team: "Prishtina", played: 1, wins: 0, losses: 1, scored: 0, conceded: 20 }, { team: "Peja", played: 1, wins: 1, losses: 0, scored: 20, conceded: 0 }]);
  assert.ok(early.probabilities.home > .3 && early.probabilities.home < .5);
});
