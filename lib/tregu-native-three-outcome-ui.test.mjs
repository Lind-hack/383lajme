...
const marketChart = readFileSync(
  new URL("../components/tregu/market-chart.tsx", import.meta.url),
  "utf8"
);
...
test("exact chart preserves recorded timestamps while carrying values to a live edge", () => {
  assert.match(exactChart, /item\.points\.map\(\(point\) => point\.t\)/);
  assert.match(exactChart, /selectRecordedRange\(series, showRanges \? range : "Gjithë", visibleEnd\)/);
  assert.match(exactChart, /const cadence = drawsLive \? 1_000 : 60_000/);
  assert.match(exactChart, /cleaned\.some\(\(item\) => item\.displayPoints\.length >= 2\)/);
  assert.match(exactChart, /displayPoints\.length >= 2/);
  // Every curve is drawn through the recorded points only; step is the default.
  assert.match(exactChart, /angularRecordedPath\(points, model\.x, model\.y\)/);
  assert.match(exactChart, /stepRecordedPath\(points, model\.x, model\.y\)/);
  assert.match(exactChart, /curve = "step"/);
  assert.match(exactChart, /displayPoints\.length >= 1/);
  assert.match(exactChart, /item\.hold/);
  assert.match(exactChart, /Pa të dhëna të regjistruara në këtë interval/);
  assert.match(exactChart, /Linja shfaqet sapo të regjistrohet një vlerë/);
  assert.doesNotMatch(exactChart, /Ende pa lëvizje në këtë interval|Vija shfaqet pas ndryshimit të dytë të regjistruar/);
  assert.doesNotMatch(exactChart, /Math\.random|dramatize/);
});

test("generic market chart renders from one persisted trade instead of requiring two", () => {
  assert.match(marketChart, /if \(data\.realCount < 1\)/);
  assert.doesNotMatch(marketChart, /if \(data\.realCount < 2\)/);
});
...
test("market API exposes exact outcome tape, traded volume and persisted freshness", () => {
  ...
  assert.match(marketsApi, /delta7d: hasOutcomeBook \? null : delta7d/);
