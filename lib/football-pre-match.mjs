import { applyRosterNews } from "./sport-pregame-context.mjs";
...
export function buildFootballOpeningModel(fixture, { outcomeKeys = ["home", "draw", "away"] } = {}) {
  ...
  const baseProbabilities = normalize(Object.fromEntries(keys.map((key) => [key, 0.72 * normalizedBase[key] + 0.28 * model[key]])), keys);
  const probabilities = bookmaker ? baseProbabilities : applyRosterNews(baseProbabilities, fixture.team_news);
  ...
  return {
    ...
    sources: [...sources, ...(fixture.team_news ?? [])],
    team_news: fixture.team_news ?? [],
    method: bookmaker
