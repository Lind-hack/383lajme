...
const FOOTBALL_METRIC_ROWS = [
  { key: "xg", label: "Golat e pritshëm (xG)", digits: 2 },
  { key: "possession", label: "Posedimi i topit", suffix: "%" },
  { key: "shots", label: "Gjuajtjet totale" },
  { key: "shots_on_target", label: "Gjuajtjet në portë" },
  { key: "corners", label: "Goditje nga këndi" },
  { key: "big_chances", label: "Shanse të mëdha të krijuara" },
  { key: "yellow_cards", label: "Kartonë të verdhë" },
  { key: "red_cards", label: "Kartonë të kuq" },
];

/** Build only rows backed by at least one supplied provider observation. */
export function buildFootballMetricRows(homeMetrics = {}, awayMetrics = {}) {
  return FOOTBALL_METRIC_ROWS.flatMap(({ key, label, digits, suffix }) => {
    const homeSupplied = Number.isFinite(Number(homeMetrics[key]));
    const awaySupplied = Number.isFinite(Number(awayMetrics[key]));
    if (!homeSupplied && !awaySupplied) return [];
    const home = homeSupplied ? Number(homeMetrics[key]) : 0;
    const away = awaySupplied ? Number(awayMetrics[key]) : 0;
    const format = (value, supplied) => {
      if (!supplied) return "—";
      if (Number.isInteger(digits)) return value.toFixed(digits);
    ...
    return [{
      ...
      home,
      away,
      homeText: format(home, homeSupplied),
      awayText: format(away, awaySupplied),
    }];
  });
}
