import { matchTopic, scoreTopic, MIN_TOPIC_SCORE } from "./dosje-match.mjs";
/**
...
export function articleMatchesTopic(article, topic) {
  ...
  // The same rule that decides whether a dossier is shown at all. This one
  // chooses the recent coverage listed inside it, and a looser test here would
  // put a NATO summit in Ankara under the Kosovo file's own reporting — the
  // wrong answer arriving by a side door.
  const { score, vetoed } = scoreTopic(article, topic);
  return !vetoed && score >= MIN_TOPIC_SCORE;
}
