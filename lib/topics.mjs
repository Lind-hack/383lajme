...
const STOP = new Set([
  "dhe", "per", "nga", "mbi", "nen", "tek", "the", "nje", "kete", "kjo", "ky",
  "sipas", "pas", "para", "gjate", "deri", "mes", "midis", "qe", "eshte",
  "jane", "ishte", "ishin", "ne", "te", "i", "e", "se", "me", "si", "u", "u",
]);
 ...
 */
function imageForMilestone(milestone, pool, index) {
  if (!pool.length) return null;

  const want = keywords(`${milestone.title} ${milestone.tag ?? ""}`);
  let best = null;
  let bestScore = 0;
  for (const a of pool) {
    const have = keywords(`${a.title ?? ""} ${a.excerpt ?? ""}`);
    let score = 0;
  ...
  // Two shared significant words means the photograph is about this subject.
  // Below that it is only from the same dossier, which is still true and still
  // useful — every entry carries a picture, and the caption says plainly that
  // it illustrates rather than documents. Assignment is by position so a
  // dossier does not repeat one photograph down its whole length, and it is
  // deterministic, so the page does not reshuffle between renders.
  const chosen = bestScore >= 2 ? best : pool[index % pool.length];
  if (!chosen?.imageUrl) return null;
  return {
    imageUrl: chosen.imageUrl,
    imageCredit: chosen.source ?? null,
    imageSlug: chosen.slug ?? null,
    imageIsSubject: bestScore >= 2,
  };
}
...
export function timelineFor(slug, articles, currentSlug) {
  ...
  // Only this dossier's own coverage is eligible, so a picture is never
  // borrowed from an unrelated subject.
  const pool = articlesForTopic(slug, articles).filter((a) => a.imageUrl);

  const milestones = (topic.milestones ?? []).map((m, i) => ({
    kind: "milestone",
    id: "m:" + m.date + ":" + m.title,
    ...m,
    ...(imageForMilestone(m, pool, i) ?? {}),
  }));
