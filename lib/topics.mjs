...
function assignImages(milestones, articles, topic) {
  const pool = (articles ?? []).filter((a) => a?.imageUrl);
  if (!pool.length) return new Map();

  const inTopic = new Set(articlesForTopic(topic.slug, pool).map((a) => a.slug));
  ...
  milestones.forEach((m, mi) => {
    ...
    pool.forEach((art, ai) => {
      ...
      // Coverage of this dossier outranks an equally-worded article from
      // elsewhere, so a picture is only borrowed from outside when it must be.
      if (inTopic.has(art.slug)) score += 1.5;
  ...
  for (const { mi, ai, score } of pairs) {
    ...
    out.set(mi, {
      ...
      imageCredit: art.source ?? null,
      imageSlug: art.slug ?? null,
      // Two shared words beyond the topic bonus means the photograph is about
      // this moment rather than merely from the same file.
      imageIsSubject: score - (inTopic.has(art.slug) ? 1.5 : 0) >= 2,
    });
    takenArticle.add(ai);
  }
  return out;
