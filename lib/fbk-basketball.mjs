...
/** Fixture and table spellings drift (sponsor prefixes, diacritics). */
export function normalizeFbkTeam(value) {
  return String(value ?? "").normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase()
    .replace(/\b(kb|bc|kk|sigal|procredit)\b/g, "").replace(/[^a-z0-9]/g, "");
}

// Kosovo Superliga: about three points of home court, and a 40-minute game's
// final margin spreads roughly twelve points either side of expectation.
const FBK_HOME_ADVANTAGE = 3;
const FBK_MARGIN_SD = 12;
// Early records are mostly noise: a team's rating counts fully only after it
// has played this many games more than zero (6 games = half weight).
const FBK_RATING_SHRINK_GAMES = 6;

function normalCdf(x) {
  // Abramowitz–Stegun 7.1.26; error below 1.5e-7 is far inside a 3–97% clamp.
  const t = 1 / (1 + 0.3275911 * Math.abs(x) / Math.SQRT2);
  const erf = 1 - t * (0.254829592 + t * (-0.284496736 + t * (1.421413741 + t * (-1.453152027 + t * 1.061405429)))) * Math.exp(-x * x / 2);
  return x >= 0 ? (1 + erf) / 2 : (1 - erf) / 2;
}

/** Points per game above an average team, shrunk toward zero while the sample is small. */
function fbkRating(row) {
  const hasPoints = Number.isFinite(row.scored) && Number.isFinite(row.conceded);
  // Without points, a smoothed win rate stands in: a 75% team is about +6 a game.
  const raw = hasPoints ? (row.scored - row.conceded) / row.played : ((row.wins + 2) / (row.played + 4) - .5) * 24;
  return raw * row.played / (row.played + FBK_RATING_SHRINK_GAMES);
}

/** Point-margin model from the federation table, explicitly distinct from a provider moneyline. */
export function fbkOpeningModel(event, standings, { homeAdvantage = FBK_HOME_ADVANTAGE } = {}) {
  const find = name => standings.find(row => row.team === name) ?? standings.find(row => normalizeFbkTeam(row.team) === normalizeFbkTeam(name));
  const home = find(event.home_team);
  const away = find(event.away_team);
  ...
  const expectedMargin = fbkRating(home) - fbkRating(away) + homeAdvantage;
  const probability = Math.max(.03, Math.min(.97, normalCdf(expectedMargin / FBK_MARGIN_SD)));
  return { probabilities: { home: probability, away: 1 - probability }, source_url: event.source_url,
    method: "FBK standings: shrunk point margin + home court; not bookmaker odds",
    inputs: { home, away, expected_margin: Math.round(expectedMargin * 10) / 10 } };
}
...
/** A game this long after tip-off with both scores published by the federation is over. */
export const FBK_FINAL_AFTER_MS = 2 * 3600000;

/**
 * The federation's results page is the league's official record. Both scores
 * must be published, unequal (basketball has no draws; 20–0 is a forfeit) and
 * the game must be well past any possible overtime.
 */
export function fbkOfficialFinal(event, now = new Date()) {
  const home = event?.home_score, away = event?.away_score;
  if (!Number.isInteger(home) || !Number.isInteger(away) || home === away) return null;
  if (now.getTime() - Date.parse(event.kickoff) < FBK_FINAL_AFTER_MS) return null;
  return { home, away };
}

/** A fixture page supplies schedule observations, and a final once the federation publishes one. */
export function normalizeFbkFixture(event, now = new Date(), { allowFinal = true } = {}) {
  const scheduled = Date.parse(event.kickoff) > now.getTime();
  const final = allowFinal ? fbkOfficialFinal(event, now) : null;
  if (final) {
    return {
      provider: "fbk", event_id: event.event_id, league: "fbk.kosovo", sport: "basketball",
      kickoff: event.kickoff, source_url: event.source_url,
      status: "STATUS_FINAL", detail: "Përfunduar — rezultati zyrtar i FBK",
      has_official_score: true, metrics: {},
      competitors: [{ team: event.home_team, homeAway: "home", score: final.home }, { team: event.away_team, homeAway: "away", score: final.away }],
      supplemental: { fbk: { availability: "federation_final", live_stats_url: event.live_stats_url, observed_scores: { home: event.home_score, away: event.away_score } } },
    };
  }
  return {
    provider: "fbk", event_id: event.event_id, league: "fbk.kosovo", sport: "basketball",
