...
/**
 * How much article text the model may read in total, and the floor and ceiling
 * for any single source.
 *
 * This is a budget rather than a fixed per-source cap, and the difference is
 * not cosmetic. A news article is written as an inverted pyramid: the scale of
 * the event first, the explanation of it last. A fixed 1400-character cut
 * therefore removes the cause from almost every article while keeping the
 * casualty figures — so a reader asking "pse po ndodh ky shpërthim" got a
 * fluent, correctly cited summary of how bad the outbreak was and not one word
 * about why, because the sentence naming the virus strain, the missing vaccine
 * and the nearby conflict sat 200 characters past the cut.
 *
 * With one source there is room for the whole piece; with six, each gets less.
 */
export const CONTEXT_CHARS = 9000;
export const SOURCE_CHARS_MIN = 1500;
export const SOURCE_CHARS_MAX = 6000;

/** The per-source share when `count` sources have to fit the budget. */
export function sourceBudget(count) {
  if (count < 1) return SOURCE_CHARS_MAX;
  const share = Math.floor(CONTEXT_CHARS / count);
  return Math.min(SOURCE_CHARS_MAX, Math.max(SOURCE_CHARS_MIN, share));
}
...
export function trimSource(article, chars = SOURCE_CHARS_MAX) {
