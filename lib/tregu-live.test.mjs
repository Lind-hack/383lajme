...
test("tregu-live cron endpoint requires a bearer secret, invokes the five-minute AI repricer, and records a distinct heartbeat", () => {
  ...
  const gate = source("lib/require-automation.ts");
  ...
  assert.match(gate, /isAutomationAuthorized/);
...
test("tregu-live uses an explicit migration allowlist and emails only verified AI market changes", () => {
  ...
  assert.match(route, /VPS outbox sender/);
  const outbox = source("scripts/send-tregu-news-movement.py");
  assert.match(outbox, /news_email_processed_at/);
  assert.match(outbox, /email_updates/);
  assert.match(outbox, /if moves:/);
...
test("official football/F1 emails require persisted before/after readback and preserve the exact state", () => {
  ...
  // A market with no competitors has no score, and saying so on every card was
  // noise. The state and the result still have to survive the readback, which
  // is what this test is actually for.
  assert.match(message.text, /Status: closed - result HOME/);
  assert.match(message.text, /Previous status: open/);
  assert.doesNotMatch(message.text, /No score supplied/);
