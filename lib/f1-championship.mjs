import { applyLiveRaceProjection, liveRaceIsProjectable, projectLiveRacePoints } from "./f1-live-championship.mjs";
...
async function api(path, fetchImpl) {
  const run = async () => {
    ...
    for (let attempt = 0; attempt < 3; attempt += 1) {
      const response = await fetchImpl(`${OPEN_F1}/${path}`, { cache: "no-store", headers: await openf1Headers({ fetchImpl }) });
...
export function buildChampionshipModel(context, { simulations = 5000 } = {}) {
  ...
  if (decision.decided) {
    ...
    for (let simulation = 0; simulation < simulations; simulation += 1) {
      ...
      // Current points already seed the championship ledger. Reusing them as
      // a large input to every future race double-counted the leader's margin
      // and produced implausible 99% forecasts. A per-simulation latent term
      // also represents upgrades, setup swings and driver form uncertainty.
      const seasonSwing = new Map(features.map((feature) => [feature.driverNumber, normal(random) * 0.72]));
      for (const event of eventModels) {
        const scored = features.map((feature) => {
          ...
          const strength =
            ...
            feature.pace * 0.20 +
            feature.speed * 0.10 +
            feature.constructorIndex * 0.14 +
            trackIndex * 0.20 +
            feature.reliability * 0.11;
          const dnfRisk = 0.018 + (1 - feature.reliability) * 0.18;
          const retired = random() < dnfRisk;
          return { feature, score: retired ? -20 + random() : strength * 2.35 + (seasonSwing.get(feature.driverNumber) ?? 0) + gumbel(random) * 1.15 };
  ...
  return {
    ...
    model: {
      version: "championship-monte-carlo-v3",
      simulations,
...
export async function fetchOpenF1ChampionshipContext({ now = new Date(), fetchImpl = fetch, liveRace = null } = {}) {
  ...
  const [latestDrivers, recentDriverLists, standings, teamStandings, latestRaceResult] = await Promise.all([
    api(`drivers?session_key=${latestRace.session_key}`, fetchImpl),
    Promise.all(recentSessions.map((session) => api(`drivers?session_key=${session.session_key}`, fetchImpl).catch(() => []))),
  ...
  const rosterByCode = new Map([...recentDriverLists.flat(), ...latestDrivers].map((driver) => [String(driver.name_acronym ?? "").toUpperCase(), driver]));
  // Reserve appearances must not replace the season's contracted race seat in
  // the championship book. Hadjar remains in; Tsunoda is not a 2026 entrant.
  rosterByCode.delete("TSU");
  const drivers = [...rosterByCode.values()];
  ...
  // The race on track, scored onto the table. Absent from `completed` and from
  // `upcomingRaces` alike, so this is the only place it can be counted, and it
  ...
  const projectedStandings = projection.standings;

  const stateKey = JSON.stringify({
    latest: latestRace.session_key,
    standings: projectedStandings.map((row) => [row.driver_number, row.points_current, row.position_current]),
    remaining: remainingEvents.map((event) => [event.session.session_key, event.sprint]),
    live: projection.applied ? [liveRace?.session?.session_key ?? null, liveRace?.lap ?? null] : null,
  });
  return {
    year,
    ...
    liveProjection: {
      ...
      session_key: projection.applied ? liveRace?.session?.session_key ?? null : null,
      lap: projection.applied ? liveRace?.lap ?? null : null,
    },
    teamStandings,
    latestRaceResult,
    recentRaces,
...
export async function buildCurrentF1ChampionshipMarket(options = {}) {
  // OpenF1 throws on its first call for the entire duration of any session —
  // "meetings?year=2026: 401" — and a Grand Prix weekend is four days of them.
  // That is why the title book could sit unrepriced for days: this function was
  // being called on schedule and throwing before it read anything. The free
  // context carries points and the remaining calendar, which is what decides a
  // championship; only lap-level pace is lost, and that degrades to neutral.
  let context;
  try {
    context = await fetchOpenF1ChampionshipContext(options);
  } catch (error) {
    const { buildFreeChampionshipContext } = await import("./f1-livetiming.mjs");
    context = await buildFreeChampionshipContext({
      year: options.now instanceof Date ? options.now.getUTCFullYear() : new Date().getUTCFullYear(),
      fetchImpl: options.fetchImpl,
      liveRace: options.liveRace ?? null,
    });
...
export function buildChampionshipMarketTemplate(championship, { now = new Date() } = {}) {
  ...
  return {
    ...
    description: "Tregu sezonal F1 përditësohet nga renditja, forma e fundit, ritmi, shpejtësia, besueshmëria, makina dhe profili i pistave të mbetura. Simulimi qëndron aktiv derisa titulli të jetë matematikisht i vendosur.",
    category: "Sport",
    status: "open",
    ...
    live_score_state: {
      key: `${championship.stateKey}:${championship.model.version}`,
