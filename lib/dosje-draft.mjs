...
export function validateMilestoneDraft(raw, { sources = [], now = new Date() } = {}) {
  ...
  //
  // Matched by content, not by character. Requiring the cited sentence to equal
  // a summary sentence exactly refused every real draft: a model reproducing
  // its own prose drifts by a comma, a dash or a space, and that drift says
  // nothing about whether the claim is sourced. The test is therefore whether
  // the model pointed at this content — containment either way, or most of the
  // same words. A sentence it never mentioned still shares almost nothing and
  // is still refused, which is the case that matters.
  const claimTexts = claims.map((c) => norm(c?.sentence)).filter(Boolean);
  const covers = (sentence) => {
    ...
    for (const c of claimTexts) {
      ...
      const have = new Set(c.split(" ").filter((w) => w.length > 3));
      let hit = 0;
      for (const w of words) if (have.has(w)) hit += 1;
      if (hit / words.size >= 0.8) return true;
    }
    return false;
  };
  ...
  // ── the quote has to be in the source it points at ────────────────────────
  //
  // A quote is what turns a link into evidence: it puts the supporting sentence
  // next to the claim, so approving is reading rather than trusting. But a
  // quote the model composed would be a new fabrication surface — a convincing
  // sentence attributed to Reuters that Reuters never wrote — so every quote is
  // checked against the text that was actually fetched, and dropped if it is
  // not there. Absent is fine; invented is not.
  ...
  for (const c of claims) {
    ...
    for (const i of c?.source_indexes ?? []) {
      const src = sources[i];
      if (!src) continue;
      if (norm(src.text).includes(q)) {
        if (!quoteFor.has(i)) quoteFor.set(i, String(c.quote).trim());
        break;
      }
    }
  }
  ...
  return {
    ...
    citations: [...used].map((i) => {
      ...
      return {
        ...
        // Where the text was actually read when the live url had died. Null for
        // a source that served itself. Carried so a reader following a citation
        // to a dead url is handed the snapshot instead of a 404.
        archive_url: s.archive_url ?? null,
        source_date: s.published_date ?? null,
        // Only a quote found verbatim in the fetched text reaches this point.
        quote: quoteFor.get(i) ?? null,
      };
    }),
  };
}
