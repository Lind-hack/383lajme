...
export function normalizeEspnSummary(config, body, fetchedAt = new Date()) {
  ...
  return { provider: "espn", event_id: String(config.event_id), league: String(config.league), sport: String(config.sport ?? "soccer"), date: competition.date, kickoff: competition.date,
    status: String(status.name), detail: String(status.shortDetail ?? status.detail ?? status.name), competitors, has_official_score, metrics, starting_lineups, football_format,
...
function stateKey(event) {
  return JSON.stringify({ status: event.status, detail: event.detail, competitors: event.competitors.map(({ team, score, winner, advance, aggregate_score, shootout_score, series_winner }) => ({ team, score, winner, advance, aggregate_score, shootout_score, series_winner })).sort((a, b) => a.team.localeCompare(b.team)), metrics: event.metrics, metric_sources: event.metric_sources, starting_lineups: event.starting_lineups, football_format: event.football_format, flashscore: { availability: event.supplemental?.flashscore?.availability, source_url: event.supplemental?.flashscore?.source_url, reason: event.supplemental?.flashscore?.reason } });
}
...
function finalPrices(market, event) {
  ...
  const isQualification = format?.marketIntent === "to_qualify" || !(market.sport_outcomes ?? []).some((outcome) => outcome.key === "draw");
...
function liveFootballReference({ market, event, base, difference: suppliedDifference = null }) {
  ...
  const minute = clamp(minutesFromDetail(event.detail), 0, 90);
  const difference = Number.isFinite(Number(suppliedDifference)) ? Number(suppliedDifference) : Number(home.score) - Number(away.score);
  if (drawKey && difference === 0) {
    // As time expires without a goal, the draw becomes the dominant state.
    // Team strength only splits the remaining chance of a late winner.
    const draw = clamp(0.44 + minute * 0.0032, 0.44, 0.73);
    const winMass = 1 - draw;
    return normalized({ [homeKey]: winMass * priorShare, [drawKey]: draw, [awayKey]: winMass * (1 - priorShare) });
  }
  if (difference !== 0) {
    const leader = difference > 0 ? homeKey : awayKey;
    const trailer = difference > 0 ? awayKey : homeKey;
    const leaderProbability = clamp(0.58 + minute * 0.003 + Math.min(0.14, (Math.abs(difference) - 1) * 0.08), 0.58, 0.92);
    const comebackMass = 1 - leaderProbability;
    const drawMass = drawKey ? clamp(comebackMass * (0.72 - minute * 0.002), 0.03, comebackMass * 0.78) : 0;
    return drawKey
      ? normalized({ [leader]: leaderProbability, [drawKey]: drawMass, [trailer]: comebackMass - drawMass })
      : normalized({ [leader]: leaderProbability, [trailer]: comebackMass });
  }
  return base;
}
...
export function buildSportMarketPlan({ markets, events, verifiedNews = [], now = new Date() }) {
  ...
  return (markets ?? []).flatMap((market) => {
    ...
    let reference = liveFootballReference({ market, event, base: opening, difference });
    ...
    const cap = difference ? 0.10 : (Object.keys(event.metrics ?? {}).length ? 0.04 : (scoredNews.length ? 0.03 : 0.02));
