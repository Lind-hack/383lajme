...
const FINAL_STATUS = /(?:FINAL|FULL_TIME|POST)/i;
...
export function normalizeEspnSummary(config, body, fetchedAt = new Date()) {
  ...
  const competitors = (competition?.competitors ?? []).map((competitor) => ({
    ...
    aggregate_score: Number.isFinite(Number(competitor?.aggregateScore))
      ...
      : null,
    shootout_score: Number.isFinite(Number(competitor?.shootoutScore))
  ...
  for (const competitor of competitors) {
    ...
    if (!series) continue;
    if (competitor.aggregate_score === null && Number.isFinite(Number(series.aggregateScore))) {
      competitor.aggregate_score = Number(series.aggregateScore);
    }
  ...
  return { provider: "espn", event_id: String(config.event_id), league: String(config.league), sport: String(config.sport ?? "soccer"), date: competition.date, kickoff: competition.date,
    period: Number(competition?.status?.period), clock_seconds: Number(competition?.status?.clock), status: String(status.name), detail: String(status.shortDetail ?? status.detail ?? status.name), competitors, has_official_score, metrics, starting_lineups, football_format,
...
function stateKey(event) {
  return JSON.stringify({ status: event.status, detail: event.detail, period: event.period, clock_seconds: event.clock_seconds, competitors: event.competitors.map(({ team, score, winner, advance, aggregate_score, shootout_score, series_winner }) => ({ team, score, winner, advance, aggregate_score, shootout_score, series_winner })).sort((a, b) => a.team.localeCompare(b.team)), metrics: event.metrics, metric_sources: event.metric_sources, starting_lineups: event.starting_lineups, football_format: event.football_format, flashscore: { availability: event.supplemental?.flashscore?.availability, source_url: event.supplemental?.flashscore?.source_url, reason: event.supplemental?.flashscore?.reason } });
}
