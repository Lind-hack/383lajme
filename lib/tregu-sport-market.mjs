...
function pairedBinaryReference(event) {
  ...
  const swing = (spain.score - argentina.score) * 0.14 + (spain.score === argentina.score ? 0 : Math.sign(spain.score - argentina.score) * minute * 0.003);
...
export function buildArgentinaSpainPairedBinaryPlan({ markets, events, now = new Date() }) {
  ...
  const state = { key, status: event.status, detail: event.detail, competitors: event.competitors, metrics: event.metrics, metric_sources: event.metric_sources, starting_lineups: event.starting_lineups, source_url: event.source_url, supplemental: event.supplemental, kickoff: event.kickoff, has_official_score: event.has_official_score };
  ...
  return [{
    kind: "score", event, spainMarket, argentinaMarket, state, reference, close_market: final,
