...
export function buildSportMarketPlan({ markets, events, verifiedNews = [], now = new Date() }) {
  ...
  return (markets ?? []).flatMap((market) => {
    ...
    const difference = useAggregate ? aggregateScores[0] - aggregateScores[1] : home.score - away.score;
    if (difference && homeKey && awayKey) {
      ...
      reference = drawKey
        ? normalized(Object.fromEntries(market.sport_outcomes.map((outcome) => [outcome.key, outcome.key === leader ? leaderProbability : outcome.key === drawKey ? Math.max(0.03, 0.28 - minute * 0.0025) : Math.max(0.02, 1 - leaderProbability - Math.max(0.03, 0.28 - minute * 0.0025))])))
        : normalized({ [leader]: leaderProbability, [trailer]: 1 - leaderProbability });
    }
