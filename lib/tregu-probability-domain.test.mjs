...
test("a step holds the price flat, then jumps at the moment it moved", () => {
  const path = stepRecordedPath(
    [{ t: 0, p: 0.28 }, { t: 10, p: 0.42 }, { t: 20, p: 0.3 }],
    (value) => value,
    (value) => value * 100
  );
  assert.equal(path, "M0.0 28.0 H10.0 V42.0 H20.0 V30.0");
});

test("the default range widens until it contains a move", () => {
  const day = 24 * 60 * 60_000;
  const end = 30 * day;
  // Traded three days ago, flat since: 1d and 4h are flat lines, 1w shows the buy.
  const series = [{ points: [{ t: end - 5 * day, p: 0.28 }, { t: end - 3 * day, p: 0.42 }, { t: end, p: 0.42 }] }];
  assert.equal(rangeWithMovement(series, "1d", end), "1w");
  // A move inside the last day keeps the preferred range.
  const recent = [{ points: [{ t: end - 2 * 60 * 60_000, p: 0.3 }, { t: end, p: 0.35 }] }];
  assert.equal(rangeWithMovement(recent, "1d", end), "1d");
  // Nothing ever moved: everything.
  assert.equal(rangeWithMovement([{ points: [{ t: end, p: 0.5 }] }], "1d", end), "Gjithë");
});
