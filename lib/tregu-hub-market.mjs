...
// Levante and Athletic Club both play in red, so their two lines were drawn in
// the same #C8142F: a buy on one and the matching fall of the other read as two
// identical lines sliding together. The first outcome keeps its colour; any
// later one too close to a colour already on the chart takes the first
// palette colour that is clear of all of them.
const SEPARATION_PALETTE = [HOME_FALLBACK_COLOR, "#0E7C66", "#6D3FC0", "#B45309", "#111111"];

function hexRgb(hex) {
  return /^#[0-9a-f]{6}$/i.test(String(hex)) ? [1, 3, 5].map((at) => Number.parseInt(hex.slice(at, at + 2), 16)) : null;
}

/** "Redmean" RGB distance: cheap, and close enough to perceived difference for lines. */
export function colorDistance(a, b) {
  const first = hexRgb(a);
  const second = hexRgb(b);
  if (!first || !second) return Infinity;
  const mean = (first[0] + second[0]) / 2;
  const [dr, dg, db] = first.map((value, index) => value - second[index]);
  return Math.sqrt((2 + mean / 256) * dr * dr + 4 * dg * dg + (2 + (255 - mean) / 256) * db * db);
}

export function separateOutcomeColors(colors, minDistance = 110) {
  const used = [];
  return (colors ?? []).map((color) => {
    let chosen = color;
    if (used.some((other) => colorDistance(other, chosen) < minDistance)) {
      chosen = SEPARATION_PALETTE.find((candidate) => used.every((other) => colorDistance(other, candidate) >= minDistance)) ?? color;
    }
    used.push(chosen);
    return chosen;
  });
