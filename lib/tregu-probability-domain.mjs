...
export function selectRecordedRange(series, rangeKey = "Gjithë", visibleEnd = null) {
  ...
  const latestRecorded = timestamps.length ? Math.max(...timestamps) : null;
  const requestedEnd = Number(visibleEnd);
  const end = visibleEnd != null && Number.isFinite(requestedEnd)
    ? Math.max(latestRecorded ?? requestedEnd, requestedEnd)
    : latestRecorded;
...
export function recordedRangeDisplaySeries(series, start, end) {
  return series.map((item) => {
    ...
    const lastKnown = displayPoints.at(-1);
    if (lastKnown && Number.isFinite(end) && Number(end) > lastKnown.t) {
      displayPoints.push({ t: Number(end), p: lastKnown.p, held: true });
    }
...
/**
 * A market price holds until the next trade or update moves it, so the honest
 * drawing is a step: flat, then a vertical jump at the moment of the move. A
 * diagonal between two points days apart drew a buy as a slow slide that
 * started before anyone had traded.
 */
export function stepRecordedPath(points, xFor, yFor) {
  if (!Array.isArray(points) || points.length === 0) return "";
  let path = `M${xFor(points[0].t).toFixed(1)} ${yFor(points[0].p).toFixed(1)}`;
  for (let index = 1; index < points.length; index++) {
    path += ` H${xFor(points[index].t).toFixed(1)} V${yFor(points[index].p).toFixed(1)}`;
  }
  return path;
}

/**
 * The first range, from `preferred` outward, whose window shows a move of at
 * least half a point. A market that last traded three days ago opened on "1d"
 * as three flat lines, which reads as a chart with nothing in it.
 */
export function rangeWithMovement(series, preferred = "1d", visibleEnd = null) {
  const keys = RECORDED_RANGE_OPTIONS.map((option) => option.key);
  for (const key of keys.slice(Math.max(0, keys.indexOf(preferred)))) {
    const { series: windowed } = selectRecordedRange(series, key, visibleEnd);
    const moved = windowed.some((item) => {
      const values = [item.hold?.p, ...item.points.map((point) => point.p)].filter(Number.isFinite);
      return values.length >= 2 && Math.max(...values) - Math.min(...values) >= 0.005;
    });
    if (moved) return key;
  }
  return "Gjithë";
