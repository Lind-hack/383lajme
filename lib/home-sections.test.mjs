...
// Titles as they stood on the homepage, 2026-09-26: three write-ups of one
// story each, worded too differently for the shared-word check.
test("headlines naming the same people are one story", () => {
  assert.equal(sameStoryByNames(
    "Papa Leo kërkon përdorim etik të inteligjencës artificiale",
    "Papa Leo XIV paralajmëron për humbjen e njerëzores nga AI"
  ), true);
  assert.equal(sameStoryByNames(
    "Trump dhe Xi diskutojnë tregtinë në takimin në Uashington",
    "Trump-Xi: samiti synon administrimin e rivalitetit"
  ), true);
  assert.equal(sameStoryByNames(
    "Kosovë: Kurti takon Osmanin për zgjedhjen e presidentit",
    "Kosovë: Osmani i përgjigjet Kurtit për presidentin"
  ), true);
});

test("one shared name, or only a shared place, is not enough", () => {
  assert.equal(sameStoryByNames(
    "Trump fton Putinin në G20",
    "Trump dhe Xi takohen në Uashington"
  ), false);
  assert.equal(sameStoryByNames(
    "Kosovë: Policia arreston dy në Prishtinë",
    "Kosovë: Prishtina hap sezonin e teatrit në Prishtinë"
  ), false);
});

test("the claim drops a same-names follow-up", () => {
  const first = art("Botë", "Trump dhe Xi diskutojnë tregtinë në takimin në Uashington");
  const second = art("Botë", "Trump-Xi: samiti synon administrimin e rivalitetit");
  const other = art("Botë", "Ukrainë: Sulmet ruse vrasin tetë në Harkiv");
  const ledger = createLedger([first]);
  assert.deepEqual(claim(ledger, [second, other], 5).map((a) => a.id), [other.id]);
});
