...
import {
  TRADE_SUCCESS_SOUND_ASSET,
  TRADE_SUCCESS_SOUND_DURATION_MS,
  TRADE_SUCCESS_SOUND_FADE_OUT_MS,
  TRADE_SUCCESS_SOUND_MAX_DURATION_MS,
  resolveTradeSuccessSoundProfile,
} from "./tregu-trade-sound.mjs";
...
test("trade success audio resolves sport and UEFA competition contexts", () => {
  ...
  assert.equal(resolveTradeSuccessSoundProfile({ sportTheme: "football", league: "uefa.nations" }), "nations");
  ...
  for (const league of ["eng.1", "esp.1", "ita.1", "ger.1"]) {
    assert.equal(resolveTradeSuccessSoundProfile({ league }), "football");
  }
  assert.equal(TRADE_SUCCESS_SOUND_ASSET.football, "/audio/tregu-success/football-crowd-3s-5s-v2.wav");
  assert.equal(TRADE_SUCCESS_SOUND_ASSET.f1, "/audio/tregu-success/f1-passby-2s-4s-v2.wav");
  assert.equal(TRADE_SUCCESS_SOUND_ASSET.basketball, "/audio/tregu-success/basketball-swish-5s-8s-v2.wav");
  assert.equal(TRADE_SUCCESS_SOUND_ASSET.champions, "/audio/tregu-success/champions-the-champions-13s-17s-v2.wav");
  ...
  assert.equal(TRADE_SUCCESS_SOUND_DURATION_MS.basketball, 3_000);
  assert.equal(TRADE_SUCCESS_SOUND_ASSET.conference, TRADE_SUCCESS_SOUND_ASSET.europa);
  assert.equal(TRADE_SUCCESS_SOUND_MAX_DURATION_MS, 5_000);
  assert.equal(TRADE_SUCCESS_SOUND_FADE_OUT_MS, 400);
  assert.ok(Object.values(TRADE_SUCCESS_SOUND_DURATION_MS).every((duration) => duration <= TRADE_SUCCESS_SOUND_MAX_DURATION_MS));
...
test("licensed source extraction preserves the requested clip windows and terminal fade", () => {
  const extractor = readFileSync(new URL("../scripts/extract-tregu-success-audio.mjs", import.meta.url), "utf8");
  assert.match(extractor, /champions:[\s\S]*start: 13,[\s\S]*duration: 4/);
  assert.match(extractor, /f1:[\s\S]*start: 2,[\s\S]*duration: 2/);
  assert.match(extractor, /football:[\s\S]*start: 3,[\s\S]*duration: 2/);
  assert.match(extractor, /basketball:[\s\S]*start: 5,[\s\S]*duration: 3/);
  assert.match(extractor, /const FADE_SECONDS = 0\.4/);
});
...
test("related football markets carry and render both team logos", () => {
  ...
  assert.match(route, /const sportOutcomes = Array\.isArray\(m\.sport_outcomes\)/);
...
test("related rows price a multi-outcome book instead of the dead binary one", () => {
  const route = readFileSync(new URL("../app/api/tregu/markets/[slug]/route.ts", import.meta.url), "utf8");
  const page = readFileSync(new URL("../app/tregu/[slug]/page.tsx", import.meta.url), "utf8");

  // q_yes/q_no are never written for a multi-outcome market, so lmsrPriceYes over
  // them returns exactly 0.5 for every related row. The columns that carry the
  // real book have to be selected, and priced with the same helper as the floor.
  assert.match(route, /market_type, outcome_quantities/);
  assert.match(route, /outcomeProbabilities = hasOutcomeBook/);
  assert.match(route, /lmsrSportOutcomePrices\(\{/);
  // …and the row has to say which outcome the number belongs to.
  assert.match(route, /leadOutcomeKey/);
  assert.match(page, /function relatedLeadLabel/);
  assert.match(page, /className="tregu-rel-lead"/);
});
