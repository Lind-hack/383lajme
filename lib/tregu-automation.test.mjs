...
test("reprice plan includes every open market and retains only verified non-social evidence", () => {
  const plan = buildRepricePlan({
    ...
    verifiedArticles: [
      { slug: "kuvendi-ligji", category: "Politikë", publishedAt: "2026-07-13T04:30:00.000Z", source: "Kuvendi i Kosovës" },
...
test("failed scoring records an auditable signal failure without pausing an open market", () => {
  const plan = buildRepricePlan({
    ...
    verifiedArticles: [{ slug: "kuvendi-ligji", category: "Politikë", publishedAt: "2026-07-13T04:30:00.000Z" }],
  });
...
test("verified Groq scoring moves live LMSR odds by its capped hybrid oracle adjustment", () => {
  const plan = buildRepricePlan({
    ...
    verifiedArticles: [{ slug: "iran-trump-deal", category: "Botë", publishedAt: "2026-07-13T04:30:00.000Z", source: "Reuters", url: "https://www.reuters.com/example" }],
  });
...
test("protected daily-draft context supplies recent evidence bodies and active topics", () => {
  ...
  assert.match(automation, /plan\.rows\.length < 2/);
  assert.match(automation, /insufficient_tradable_market_inventory/);
});
