...
test("news repricing rejects stale, unrelated, headline-only, and already-consumed evidence before provider scoring", () => {
  ...
  const unrelatedMarket = { ...market, id: "market-israel-lebanon", slug: "israel-liban-marreveshje", question: "Bisedimet Izrael-Liban prodhojnë marrëveshje deri më 2 shtator?", source_article_slugs: [] };
  const genericActionArticles = [
    { slug: "qatar-iran", title: "Kryeministri i Katarit shkon në Teheran për të ringjallur bisedimet", excerpt: "Palët kërkojnë marrëveshje të re.", source: "Al Jazeera", category: "Botë", publishedAt: "2026-08-29T11:50:00.000Z" },
    { slug: "albania-uk", title: "Shqipëria dhe Britania nënshkruajnë marrëveshjen e parë në rajon", excerpt: "Palët njoftuan marrëveshjen.", source: "Euronews Albania", category: "Botë", publishedAt: "2026-08-29T11:51:00.000Z" },
  ];
  assert.deepEqual(buildRepricePlan({ markets: [unrelatedMarket], verifiedArticles: genericActionArticles, now: scanNow })[0].evidence, []);

  const ceasefireMarket = {
    ...market,
    id: "market-russia-ukraine-ceasefire",
    slug: "armepushim-rusi-ukraine-4-muaj",
    category: "bote",
    question: "A do të shpallet një armëpushim në luftën midis Rusisë dhe Ukrainës brenda 4 muajve?",
    source_article_slugs: [],
  };
  const libyaElections = { slug: "libya-elections", title: "Palët rivale në Libi bien dakord për zgjedhje kombëtare brenda dy vitesh", excerpt: "Marrëveshja lidhet me zgjedhje në Libi.", source: "Al Jazeera", category: "Botë", publishedAt: "2026-08-29T11:50:00.000Z" };
  const blackSeaAttack = { slug: "ukraine-black-sea", title: "Ukraina përballet me sulme të reja në Detin e Zi", excerpt: "Sulmet rrisin tensionet ushtarake.", source: "Reuters", category: "Botë", publishedAt: "2026-08-29T11:51:00.000Z" };
  const ceasefireTalks = { slug: "russia-ukraine-ceasefire-talks", title: "Russia and Ukraine resume ceasefire talks as officials discuss a possible truce", excerpt: "Officials say the ceasefire talks address a possible halt to fighting.", source: "Reuters", category: "Botë", publishedAt: "2026-08-29T11:52:00.000Z" };
  assert.deepEqual(buildRepricePlan({ markets: [ceasefireMarket], verifiedArticles: [libyaElections, blackSeaAttack], now: scanNow })[0].evidence, []);
  assert.deepEqual(buildRepricePlan({ markets: [ceasefireMarket], verifiedArticles: [ceasefireTalks], now: scanNow })[0].evidence.map((article) => article.slug), ["russia-ukraine-ceasefire-talks"]);

  const fireMarket = {
    ...market,
    id: "market-paris-fire",
    slug: "zjarri-prane-parisit-i-qellimshem-180-dite",
    category: "bote",
    question: "A do të konfirmohet se zjarri pranë Parisit ishte i qëllimshëm brenda 180 ditëve?",
    source_article_slugs: [],
  };
  const turkeyUkraineShips = { slug: "turkey-ukraine-black-sea", title: "Turqia thërret ambasadorin ukrainas pas goditjeve ndaj dy anijeve në Detin e Zi", excerpt: "Sulmet rrisin tensionet ushtarake.", source: "Al Jazeera", category: "Botë", publishedAt: "2026-08-29T11:53:00.000Z" };
  const parisFireInvestigation = { slug: "paris-fire-investigation", title: "Investigators say the fire near Paris may have been deliberate", excerpt: "Authorities are examining whether the Paris fire was intentionally started.", source: "Reuters", category: "Botë", publishedAt: "2026-08-29T11:54:00.000Z" };
  assert.deepEqual(buildRepricePlan({ markets: [fireMarket], verifiedArticles: [turkeyUkraineShips], now: scanNow })[0].evidence, []);
  assert.deepEqual(buildRepricePlan({ markets: [fireMarket], verifiedArticles: [parisFireInvestigation], now: scanNow })[0].evidence.map((article) => article.slug), ["paris-fire-investigation"]);
});
...
test("news repricing source and deadline guards are wired into the production server", () => {
  ...
  assert.match(server, /NEWS_DEADLINE_DECAY_INTERVAL_MS/);
...
test("tregu-live market update endpoint delegates to the isolated five-minute news repricer", () => {
  ...
  const gate = readFileSync(new URL("./require-automation.ts", import.meta.url), "utf8");

  assert.match(route, /runTreguLiveAutomation/);
  // Auth lives in the shared gate now: the route must use it, and the gate must deny with 401.
  assert.match(route, /automationDenied/);
  assert.match(gate, /status:\s*401/);
...
test("daily runner uses service generation and opt-in notifications while preserving legacy command compatibility", () => {
  ...
  assert.match(runner, /generateDailyMarkets\(prompt\)/);
  assert.doesNotMatch(runner, /execFileSync\(hermesBin/);
  ...
  assert.match(runner, /closes_in_hours between 720 and 2160/);
  assert.match(runner, /Do not mechanically start/);
...
test("protected daily-draft context supplies recent evidence bodies and active topics", () => {
  ...
  assert.match(automation, /contract_version === "news-event-v3" \? "open" as const : "draft" as const/);
