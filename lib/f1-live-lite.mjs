...
function parseInlineRows(lines) {
  return lines.flatMap((line) => {
    ...
    // NOTE: every escape here must be single-backslash. This is a regex literal,
    // not a string, so `\\s` would mean "a literal backslash followed by s" and
    // the row would never match. Pits must also be a CAPTURING group, because
    // makeRow reads it from match[4].
...
export function openF1ToWinnerLeaderboard(live) {
  ...
  const rows = live.rows.map(row => ({ position: row.position, driver: row.driver, driver_code: row.driver_code, gap: row.position === 1 ? "LEADER" : `+${Number(row.gap_to_leader ?? 0).toFixed(3)}`, pits: 0, tyre: row.tyre ?? null, stint: row.stint ?? null, status: null }));
  return { source_url: live.source_url, rows, race: { status: "LIVE", current_lap: live.lap ?? null, total_laps: null }, state_key: JSON.stringify({ source: "openf1", session: live.session?.session_key, lap: live.lap, rows: live.rows, weather: live.weather, race_control: live.race_control }) };
}
