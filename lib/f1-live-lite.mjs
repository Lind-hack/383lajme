...
export function buildF1RaceWinnerPlan({ markets, leaderboard }) {
  ...
  return (markets ?? []).flatMap((market) => {
    ...
    for (const row of rows) {
      ...
      const disqualified = /DISQUAL|BLACK FLAG|RETIRED|DNF/i.test(sanction);
      const sanctionFactor = /STOP.?GO|DRIVE.?THROUGH/i.test(sanction) ? 0.12 : /TIME PENALTY|PENALTY/i.test(sanction) ? 0.45 : 1;
      const value = disqualified ? 0 : Math.exp(-0.68 * (row.position - 1) - 0.035 * gapSeconds(row.gap)) * sanctionFactor;
...
export function openF1ToWinnerLeaderboard(live) {
  ...
  const rows = live.rows.map(row => ({ position: row.position, driver: row.driver, driver_code: row.driver_code, gap: row.position === 1 ? "LEADER" : `+${Number(row.gap_to_leader ?? 0).toFixed(3)}`, pits: 0, tyre: row.tyre ?? null, stint: row.stint ?? null, status: penaltyFor(row) }));
