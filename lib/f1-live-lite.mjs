import { advanceRaceMemory } from "./f1-race-memory.mjs";
...
function rowState(row) {
  return JSON.stringify({ position: row.position, driver_code: row.driver_code, gap: row.gap, pits: row.pits, in_pit: row.in_pit ?? null, laps: row.laps ?? null, tyre: row.tyre ?? null, stint: row.stint ?? null, status: row.status ?? null });
}
...
export function parseF1LiveLiteLeaderboard(renderedText) {
  if (renderedText && typeof renderedText === "object" && Array.isArray(renderedText.tableRows)) {
    ...
    // Columns are read by their header when the render supplies one: the
    // dashboard hides columns by viewport width, so a fixed index is only the
    // fallback for the layout it was measured on (POS DRIVER GAP INT LAST BEST
    // SECTORS LAPS PIT TYRE at 1280px).
    const col = { POS: 0, DRIVER: 1, GAP: 2, INT: 3, LAST: 4, LAPS: 7, PIT: 8, TYRE: 9 };
    const headers = (renderedText.headers ?? []).map((header) => String(header ?? "").trim().toUpperCase());
    if (headers.includes("DRIVER") && headers.includes("GAP")) for (const name of Object.keys(col)) if (headers.includes(name)) col[name] = headers.indexOf(name);
    const rows = renderedText.tableRows.flatMap(cells => {
      const code = String(cells[col.DRIVER] ?? "").match(/\b[A-Z]{3}(?=\s|\d|$)/)?.[0];
      const position = /^\d+$/.test(String(cells[col.POS]).trim()) ? Number(cells[col.POS]) : null;
      const gap = String(cells[col.GAP] ?? "").trim();
      const retired = /^(DNF|DNS|DSQ)$/i.test(gap);
      if (!code || (!position && !retired)) return [];
      const pitCell = String(cells[col.PIT] ?? "").trim().toUpperCase();
      const pits = /^\d+$/.test(pitCell) ? Number(pitCell) : null;
      const tyre = String(cells[col.TYRE] ?? "").match(/SOFT|MEDIUM|HARD|INTERMEDIATE|WET/)?.[0] ?? null;
      const tyreAge = String(cells[col.TYRE] ?? "").match(/(\d+)\s*$/)?.[1];
      const laps = String(cells[col.LAPS] ?? "").trim();
      return [{ driver: code, driver_code: code, position, gap: position === 1 ? "LEADER" : gap,
        interval: String(cells[col.INT] ?? "").trim() || null, last_lap: String(cells[col.LAST] ?? "").trim() || null,
        laps: /^\d+$/.test(laps) ? Number(laps) : null, pits, in_pit: /^(IN )?PIT$/.test(pitCell),
        tyre, tyre_age: tyreAge ? Number(tyreAge) : null, status: retired ? gap.toUpperCase() : null }];
    });
    ...
    const text = String(renderedText.text ?? "");
    const rain = text.match(/\bRAIN\s+([\d.]+)\s*mm/i)?.[1];
    // The session sits on the line under the Grand Prix name ("Azerbaijan Grand
    // Prix / QUALIFYING / FINISHED"). Searching the whole page instead matched
    // "Race Pace" in the site menu and called a qualifying session a race.
    const lines = text.split(/\r?\n/).map((line) => line.trim()).filter(Boolean);
    const nameAt = lines.lastIndexOf(String(renderedText.sessionName).trim());
    const sessionKind = nameAt >= 0 ? lines[nameAt + 1]?.toUpperCase().match(/^(SPRINT QUALIFYING|SPRINT|QUALIFYING|PRACTICE \d|RACE)$/)?.[1] ?? null : null;
    return { source_url: F1_DASHBOARD_URL, rows, race, session_name: renderedText.sessionName, session_kind: sessionKind,
      weather: rain == null ? null : { rainfall: Number(rain) },
...
export async function renderOfficialF1LiveLite({ cdpUrl = process.env.F1_LIVE_LITE_CDP_URL } = {}) {
  ...
  if (!cdpUrl && !executablePath) {
    // playwright-core deliberately ships no browser, so one has to be supplied.
    //
    // This used to be gated on process.env.VERCEL. When production moved to
    // Railway that variable stopped existing, the branch stopped running, and
    // the launch below fell through to a bare "/usr/bin/chromium" that is not
    // in the image — so every call threw "Failed to launch chromium because
    // executable doesn't exist". This path is the ONLY live F1 source during a
    // session (OpenF1 refuses anonymous callers for a session's whole
    // duration), so race-winner odds silently froze at their pre-race values
    // for the length of every race, and only started moving again once the
    // session ended and the public OpenF1 endpoints reopened.
    //
    // Keyed on "no browser was supplied" rather than on a host name: the
    // bundled Chromium is the fallback everywhere, and an explicit
    // F1_LIVE_LITE_CHROMIUM_PATH or CDP URL still wins where one exists.
  ...
  try {
    ...
    return await page.evaluate(() => ({ text: document.body.innerText,
      ...
      headers: [...document.querySelectorAll('table th')].filter(cell => getComputedStyle(cell).display !== 'none').map(cell => cell.innerText.trim()),
 ...
 * an early stop may be an undercut. What it does change is the order the race is
 * priced in: a car that still owes its stop is priced a pit-lane loss further
 * back (f1-race-memory.mjs), so a leader who has not pitted is not treated as
 * clear of a rival who already has. The memory of the previous frame rides in
 * live_score_state.race_memory. */
export function buildF1RaceWinnerPlan({ markets, leaderboard, now = Date.now() }) {
  ...
  return (markets ?? []).flatMap((market) => {
    ...
    // The dashboard names the Grand Prix, not the session, so a live qualifying
    // or sprint under the same title must not price the race.
    if (leaderboard.session_kind && leaderboard.session_kind !== "RACE") return [];
    // The one-minute loop and the five-minute loop both land here; the second
    // arrival inside the same minute has nothing new to add.
    const previousState = market?.live_score_state ?? null;
    if (Date.parse(previousState?.fetched_at ?? "") > now - 40_000) return [];
    const configuredLaps = Number(cfg.total_laps) > 0 ? Number(cfg.total_laps) : null;
    const memory = advanceRaceMemory(previousState?.race_memory, {
      rows,
      race: { ...leaderboard.race, total_laps: leaderboard.race.total_laps ?? configuredLaps, safety_car: Boolean(leaderboard.race.safety_car) },
      wet: Number(leaderboard.weather?.rainfall) > 0,
      ...(Number(cfg.pit_loss_seconds) > 0 ? { pitLossSeconds: Number(cfg.pit_loss_seconds) } : {}),
    });
    const lap = Number(leaderboard.race.current_lap ?? memory.lap);
    ...
    const virtualLeader = Object.values(memory.view).find(driver => driver.virtual_position === 1);
    const paceOf = row => Number.isFinite(row?.recent_pace) ? row.recent_pace : memory.view[String(row?.driver_code ?? "").toUpperCase()]?.pace;
    const leaderPace = paceOf(rows.find(r => r.position === 1)) ?? virtualLeader?.pace;
    ...
    for (const row of rows) {
      ...
      const ownPace = paceOf(row);
      const paceDelta = Number.isFinite(ownPace) && Number.isFinite(leaderPace) ? Math.max(-2, Math.min(2, ownPace - leaderPace)) : 0;
      ...
      const view = memory.view[code];
      const gap = Number.isFinite(view?.adjusted_gap) ? view.adjusted_gap : gapSeconds(row.gap);
      const position = view?.virtual_position ?? row.position;
      ...
      const value = disqualified ? 0 : Math.exp(Math.max(-700, -rankWeight * (position - 1) - gapWeight * projectedGap - tyreRisk - wrongWetTyre)) * sanctionFactor * priorWeight;
    ...
    return [{ market, probabilities, state_key, rows, oracle_cap: 0.05,
      state: { key: state_key, source_url: leaderboard.source_url, race: leaderboard.race, rows, missing_inputs: leaderboard.missing_inputs ?? [], provider_errors: leaderboard.provider_errors ?? [],
        fetched_at: new Date(now).toISOString(), race_memory: memory.memory, current_lap: memory.lap, laps_left: memory.laps_left, total_laps: memory.total_laps,
        insights: Object.fromEntries(Object.values(memory.view).map(driver => [driver.code, driver.reason])) },
      ...
      reasoning: "Source-attributed F1 timing. Estimated race-winner model using position and gap adjusted for stops still owed, undercut/overcut tracking across refreshes, laps remaining, recent clean-lap pace, tyre age/compound, measured rain, safety-car state and sanctions. Missing telemetry stays neutral; this is not a weather radar or a calibrated guarantee." }];
  });
}
