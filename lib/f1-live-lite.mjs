...
export function buildF1RaceWinnerPlan({ markets, leaderboard }) {
  ...
  return (markets ?? []).flatMap((market) => {
    ...
    for (const row of rows) {
      ...
      const projectedGap = Math.max(0, gap + paceDelta * horizon);
    ...
    if (market?.live_score_state?.key === state_key) return [];
    return [{ market, probabilities, state_key, rows, oracle_cap: 1,
      ...
      reasoning: "Verified Formula 1 Dashboard timing. Estimated race-winner model using supplied position, gap, laps remaining, recent clean-lap pace, tyre age/compound, measured rain, safety-car state and sanctions. Missing telemetry stays neutral; this is not a weather radar or a calibrated guarantee." }];
  });
}
