...
import { shortUrl } from "./short-link.mjs";
import {
  articleUrl,
  replyMarkup,
  escapeHtml,
...
test("renderCaption bolds the title, keeps a short excerpt whole, links the article", () => {
  ...
  // A reader copying this into WhatsApp carries a link with them, and it has
  // to be short enough not to bury the headline it sits under.
  assert.ok(caption.includes(`Lexo më shumë: ${shortUrl("buxheti-2026")}`));
  assert.ok(!caption.includes("/article/"), "the canonical slug must not reach the caption");
...
test("renderCaption survives missing title and excerpt", () => {
  ...
  assert.ok(caption.includes(shortUrl("vetem-link")));
  assert.ok(caption.length <= CAPTION_LIMIT);
});
...
test("the post carries one button pointing at the short link", () => {
  const markup = replyMarkup({ slug: "buxheti-2026" });
  const rows = markup.inline_keyboard;
  assert.equal(rows.length, 1, "one row");
  assert.equal(rows[0].length, 1, "one button");
  assert.equal(rows[0][0].url, shortUrl("buxheti-2026"));
  assert.ok(rows[0][0].text.length > 0 && rows[0][0].text.length <= 64);
});

test("the button and the caption point at the same place", () => {
  // They serve different audiences — the button is for Telegram, the caption
  // line is what survives a copy into WhatsApp — so they must not drift.
  const article = { slug: "kuvendi-voton", title: "Kuvendi voton", excerpt: "Sot." };
  assert.ok(renderCaption(article).includes(replyMarkup(article).inline_keyboard[0][0].url));
});

test("a slugless article still yields a valid button rather than throwing", () => {
  const rows = replyMarkup({}).inline_keyboard;
  assert.match(rows[0][0].url, /^https:\/\/383ks\.com\/a\/[0-9a-z]{7}$/);
});
