...
export function buildTreguRepriceEmail({ runKey, changes }) {
  ...
  const cards = changes.map((change, index) => {
    ...
    const points = asSignedPoints(Number(change.after_probability) - Number(change.before_probability));
    ...
    lines.push(change.reason === "evidence_convergence" ? "Gradual movement toward the previously assessed evidence target; no new report is being claimed." : change.reason ? "Deadline-oracle update: this change was confirmed by the persisted deadline RPC." : "Evidence-backed news update: this market probability change was applied only after the verified sources above were supplied to the AI repricer.", "");
