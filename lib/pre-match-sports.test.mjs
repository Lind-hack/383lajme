...
test("F1 opening model is a normalized non-uniform 20-driver vector from verified factors", () => {
  ...
  assert.equal(model.model_version, "f1-opening-v4");
...
test("a paddock signal nudges a driver but can never outrank the grid", () => {
  const roster = Array.from({ length: 20 }, (_, index) => ({ key: `D${String(index).padStart(2, "0")}`, driver_number: index + 1, label: `Driver ${index}`, team: `Team ${index % 5}` }));
  const championshipDrivers = roster.map((driver, index) => ({ driver_number: driver.driver_number, position_current: index + 1, points_current: 300 - index * 11 }));
  const championshipTeams = Array.from({ length: 5 }, (_, index) => ({ team_name: `Team ${index}`, points_current: 500 - index * 55 }));
  const qualifying = Object.fromEntries(roster.map((driver, index) => [driver.key, index + 1]));
  ...
  const flat = buildF1RaceWinnerOpeningModel(opts);
  // The best practice pace anyone has ever reported, for the driver starting last.
  const hyped = buildF1RaceWinnerOpeningModel({ ...opts, signals: { D19: { delta: 1, reason: "fastest in every practice session" } } });

  assert.ok(hyped.probabilities.D19 > flat.probabilities.D19, "a positive signal should help");
  // But last on the grid stays last in the book.
  assert.ok(hyped.probabilities.D19 < hyped.probabilities.D00 / 50, "a signal must not overturn the grid");
  assert.equal(hyped.inputs.D19.signal_delta, 1);
  assert.ok(Math.abs(Object.values(hyped.probabilities).reduce((sum, value) => sum + value, 0) - 1) < 1e-12);
});

test("a signal outside the allowed range is clamped rather than trusted", () => {
  const roster = Array.from({ length: 20 }, (_, index) => ({ key: `D${String(index).padStart(2, "0")}`, driver_number: index + 1, label: `Driver ${index}`, team: `Team ${index % 5}` }));
  const championshipDrivers = roster.map((driver, index) => ({ driver_number: driver.driver_number, position_current: index + 1, points_current: 300 - index * 11 }));
  const championshipTeams = Array.from({ length: 5 }, (_, index) => ({ team_name: `Team ${index}`, points_current: 500 - index * 55 }));
  const qualifying = Object.fromEntries(roster.map((driver, index) => [driver.key, index + 1]));
  ...
  const clamped = buildF1RaceWinnerOpeningModel({ ...opts, signals: { D00: { delta: 99 } } });
  const atMax = buildF1RaceWinnerOpeningModel({ ...opts, signals: { D00: { delta: 1 } } });
  assert.equal(clamped.inputs.D00.signal_delta, 1);
  assert.ok(Math.abs(clamped.probabilities.D00 - atMax.probabilities.D00) < 1e-12);
});
...
test("F1 template persists the model vector and LMSR quantities rather than equal opening odds", () => {
  ...
  assert.equal(template.closes_at, "2026-08-30T19:00:00.000Z");
...
test("live football model makes a tied late match draw-dominant instead of preserving an 80% favorite", () => {
  ...
  const event = normalizeEspnSummary({ provider: "espn", event_id: "401882919", league: "esp.1", sport: "soccer" }, {
    header: { competitions: [{ date: "2026-08-27T00:00:00.000Z", status: { type: { name: "STATUS_IN_PROGRESS", shortDetail: "90+4\'" } }, competitors: [{ homeAway: "home", score: "1", team: { displayName: "Real Madrid" } }, { homeAway: "away", score: "1", team: { displayName: "Real Sociedad" } }] }] },
  ...
  assert.equal(signal.snapshot.reference_probabilities.draw > 0.90, true);
...
test("a failed automation audit is reclaimed instead of burning its run key", () => {
  const source = readFileSync(new URL("./tregu-automation-server.ts", import.meta.url), "utf8");
  // A transient OpenF1 429 used to cost the F1 template its whole
  // fifteen-minute bucket, with no retry until a fresh key was minted.
  assert.match(source, /if \(status === "failed"\)/);
  assert.ok(source.includes(`status: "running", started_at: new Date().toISOString()`));
  assert.match(source, /\.eq\("status", "failed"\)/);
  assert.match(source, /if \(reclaimed\) return \{ existing: false, run: reclaimed \}/);
});

test("an upcoming race reports no grid position instead of the array index", () => {
  const source = readFileSync(new URL("../app/api/tregu/markets/[slug]/route.ts", import.meta.url), "utf8");
  assert.match(source, /isArchived \? index \+ 1 : null/);
