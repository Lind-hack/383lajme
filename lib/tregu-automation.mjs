...
function hasStrictSubjectMatch(market, article) {
  ...
  const articleEntities = subjectTokens(articleIdentityText).filter((token) => !GENERIC_SUBJECT_TOKENS.has(token));
  const entityOverlap = articleEntities.filter((token) => marketEntities.has(token));
  if (!entityOverlap.some((token) => token.length >= 4)) return false;

  const marketActions = subjectActionGroups(marketText);
  if (!marketActions.size) return true;
