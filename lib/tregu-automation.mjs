...
const WORLD_AUDIENCE_TERMS = [
  "gaza", "hamas", "izrael", "iran", "oman", "hormuz", "ukrain", "rusi", "mosk", "putin", "trump", "biden", "shba", "shtetet e bashkuara", "nato", "europ", "kina", "kine", "tajvan", "luft", "sanksion", "tarif", "naft", "elon musk", "openai",
];
const PUBLIC_DRAMA_TERMS = [
  "arrest", "protest", "sulm", "zjar", "vras", "viktim", "vdekur", "evaku", "luft", "kriz", "skandal", "gjykat", "shperth", "shperthim", "konflikt", "bisedim", "marrëvesh", "marrevesh", "sanksion", "tarif", "korrups",
];
...
/**
 * @param {{ candidates?: any[], existingQuestions?: string[], now: Date, audienceArticles?: any[], requireMassAudience?: boolean, nonSportOnly?: boolean }} options
 */
export function buildDailyDraftPlan({ candidates, existingQuestions, now, audienceArticles = [], requireMassAudience = false, nonSportOnly = false }) {
  ...
  for (const candidate of candidates ?? []) {
    ...
    if (nonSportOnly && audienceTier === "sport") {
...
function relevantArticles(market, verifiedArticles) {
  ...
  return (verifiedArticles ?? []).filter((article) => {
    ...
    return sourceSlugs.has(article.slug) || (categories.includes(article.category) && hasStrictSubjectMatch(market, article));
  });
}
...
function hasStrictSubjectMatch(market, article) {
  ...
  const marketTokens = new Set(subjectTokens(marketText));
  const articleTokens = subjectTokens(`${article?.title ?? ""} ${article?.excerpt ?? ""} ${article?.slug ?? ""}`);
  const overlap = articleTokens.filter((token) => marketTokens.has(token));
  return overlap.length >= 2 || overlap.some((token) => token.length >= 6);
}
