...
export function newsDeadlineAction(market, now = new Date()) {
  ...
  if (closesAt <= now.getTime() + 24 * 60 * 60 * 1000) return "decay";
  return null;
}
...
function relevantArticles(market, verifiedArticles) {
  ...
  return (verifiedArticles ?? []).filter((article) => {
    ...
    const isExternalLiveHeadline = article?.verification === "external_google_news";
    if (isExternalLiveHeadline && !String(article?.url ?? "").trim()) return false;
    return sourceSlugs.has(article.slug) || ((categories.includes(article.category) || isExternalLiveHeadline) && hasStrictSubjectMatch(market, article));
  });
}
