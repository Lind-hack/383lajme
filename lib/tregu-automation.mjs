...
const NEWS_EVIDENCE_FUTURE_SKEW_MS = 5 * 60 * 1000;
...
function hasStrictSubjectMatch(market, article) {
  ...
  const marketEntities = new Set(subjectTokens(marketText).filter((token) => !GENERIC_SUBJECT_TOKENS.has(token)));
  const articleEntities = subjectTokens(`${article?.title ?? ""} ${article?.excerpt ?? ""} ${article?.slug ?? ""}`)
    .filter((token) => !GENERIC_SUBJECT_TOKENS.has(token));
  const entityOverlap = articleEntities.filter((token) => marketEntities.has(token));
  // One named entity is sufficient when it is a stable person, place, company,
  // institution, or conflict token. Generic action overlap is deliberately not.
  return entityOverlap.some((token) => token.length >= 4);
}
