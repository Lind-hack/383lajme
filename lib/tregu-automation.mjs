...
import { NEWS_CONTRACT_VERSION, isEventReviewMarket } from "./tregu-news-contract.mjs";
...
export const NEWS_DEADLINE_MIN_WINDOW_HOURS = 7 * 24;
export const NEWS_DEADLINE_MAX_WINDOW_HOURS = 14 * 24;
...
function draftViolation(candidate, context = {}) {
  const eventContract = candidate?.contract_version === NEWS_CONTRACT_VERSION;
  ...
  if (!eventContract && !/deri\s+m[ëe]\s+\d{1,2}\s+\p{L}+/iu.test(question)) return "Çdo draft duhet të ketë afat konkret në titull.";
  ...
  if (isOfficialLiveEvent) {
    ...
    if (eventContract ? closesInHours < 720 || closesInHours > 2160 : closesInHours < 6 || closesInHours > 168) return eventContract ? "Afati duhet të jetë 30 deri në 90 ditë." : "Tregjet e lajmeve të fundit duhet të mbyllen brenda 6 orësh deri në 7 ditë.";
...
export function validateDailyDraftSubmission(candidates, knownSourceSlugs, options = {}) {
  ...
  const maximum = options.nonSportOnly ? 6 : 5;
  if (!Array.isArray(candidates) || candidates.length < minimum || candidates.length > maximum) {
...
export function buildDailyDraftPlan({ candidates, existingQuestions, existingMarkets = [], now, audienceArticles = [], requireMassAudience = false, nonSportOnly = false }) {
  ...
  for (const candidate of candidates ?? []) {
    ...
    const preMatchAnalysis = quality ? {
      contract_version: candidate.contract_version === NEWS_CONTRACT_VERSION ? NEWS_CONTRACT_VERSION : DAILY_MARKET_CONTRACT_VERSION,
      ...(candidate.contract_version === NEWS_CONTRACT_VERSION ? { proposition: { ...candidate.proposition, review_at: closesAt } } : {}),
...
export function dailyDraftPublicationReason(plan) {
  ...
  if (plan.rows.length < (plan.rows.some(row => row.pre_match_analysis?.contract_version === NEWS_CONTRACT_VERSION) ? 1 : 2)) return "insufficient_tradable_market_inventory";
...
export function isEligibleNewsDeadlineMarket(market) {
  ...
  return market?.status === "open"
    && !isEventReviewMarket(market)
    && market?.market_type === "binary"
    && market?.market_classification === "general_news"
    && !["sport", "f1", "football"].includes(category);
}

/** Choose a 7–14 day countdown from the market's total lifetime. */
export function newsDeadlineDecayStartHours(market) {
  const closesAt = new Date(market?.closes_at ?? "").getTime();
  const openedAt = new Date(market?.created_at ?? market?.createdAt ?? market?.opened_at ?? "").getTime();
  if (!Number.isFinite(closesAt)) return NEWS_DEADLINE_MIN_WINDOW_HOURS;
  const totalHours = Number.isFinite(openedAt) && closesAt > openedAt
    ? (closesAt - openedAt) / HOURS_TO_MS
    : NEWS_DEADLINE_MIN_WINDOW_HOURS;
  // Short markets use the full seven-day minimum (or their whole lifetime if
  // shorter); long markets use fourteen days. Between those limits, the window
  // scales with the market's total lifetime rather than a fixed three-day rule.
  return Math.min(NEWS_DEADLINE_MAX_WINDOW_HOURS, Math.max(NEWS_DEADLINE_MIN_WINDOW_HOURS, totalHours * 0.25));
}
...
export function newsDeadlineAction(market, now = new Date()) {
  ...
  const startHours = newsDeadlineDecayStartHours(market);
  if (closesAt <= now.getTime() + startHours * HOURS_TO_MS) return "decay";
  return null;
}
...
export function newsDeadlineDecayCap(remainingHours) {
  ...
  if (!Number.isFinite(hours) || hours > NEWS_DEADLINE_MAX_WINDOW_HOURS || hours <= 0) return null;
  ...
  if (hours <= 6) return 0.015;
  if (hours <= 24) return 0.01;
  if (hours <= 72) return 0.006;
  if (hours <= 120) return 0.004;
  return 0.0025;
}
...
function relevantArticles(market, verifiedArticles, now = new Date(), usedEvidence = new Set()) {
  ...
  return (verifiedArticles ?? []).filter((article) => {
    ...
    const structured = Array.isArray(market?.pre_match_analysis?.proposition?.entities);
    if (!(pinned || categoryMatches || structured || article.verification === "original_page_extracted") || !hasStrictSubjectMatch(market, article)) return false;
    // Invalid/unrelated duplicates must not consume the identity and hide a
    // later record carrying the same publisher URL with the correct topic.
...
const SUBJECT_ACTION_GROUPS = [
  { key: "agreement", terms: ["marrevesh", "agreement", "pact", "protocol", "protokoll", "ngadales", "slowdown", "slow down", "safety standards"] },
  ...
  { key: "price-threshold", terms: ["cmimi", "price", "exceed", "above", "kaloj"] },
...
function hasStrictSubjectMatch(market, article) {
  ...
  const namedEntities = market?.pre_match_analysis?.proposition?.entities;
  if (Array.isArray(namedEntities) && namedEntities.length) {
    const headline = `${article?.title ?? ""} ${article?.excerpt ?? ""}`;
    const fullText = `${headline} ${article?.body ?? ""}`;
    const names = namedEntities.map(String).filter(name => name.length >= 2);
    // Contract entities are names, whereas question tokens include dates,
    // adjectives and slug suffixes that no relevant headline can repeat.
    const principals = names.filter(name => containsNamedEntity(market.question, name));
    if (!names.some(name => containsNamedEntity(headline, name)
      || (name.includes(" ") && name.split(" ").at(-1).length >= 4 && containsNamedEntity(headline, name.split(" ").at(-1))))) return false;
    if (principals.length && !principals.every(name => containsNamedEntity(fullText, name))) return false;
    const actions = subjectActionGroups(`${market.question} ${market.pre_match_analysis.proposition.decision}`);
    const articleActions = subjectActionGroups(fullText);
    return actions.size > 0 && [...actions].some(action => articleActions.has(action));
  }
  ...
  if (!entityOverlap.length) return false;
  if (bilateral) {
    const principals = (String(market.question).match(/\b[A-ZÇË][a-zA-ZçëÇË-]{2,}\b/g) ?? [])
      .flatMap(subjectEntityTokens).filter(token => !["will", "brenda", "gjate"].includes(token));
    const fullEntities = subjectEntityTokens(`${articleIdentityText} ${article?.body ?? ""}`);
    if (principals.length < 2 || !principals.every(token => fullEntities.includes(token))) return false;
  }
  ...
  return (markets ?? [])
    ...
    .map(({ market, evidence }) => {
      return {
        ...
        scoreSuccess(score) {
          if (!Number.isFinite(Number(score.probability))) throw new Error("Non-finite news probability");
          ...
          return {
            status: "open",
            ...(score.evidence_level === "decisive" && citedFreshSlugs.length >= 2 && evidenceSources.length >= 2
              && citedEvidence.every(article => !article.truncated)
              && ((score.resolution_action === "settle_po" && probability >= 0.99)
                || (score.resolution_action === "settle_jo" && probability <= 0.01))
              ? { settlement: { outcome: score.resolution_action === "settle_po" ? "PO" : "JO", evidence_slugs: evidenceSlugs } }
              : {}),
            marketUpdate: {
              id: market.id,
              status: "open",
 ...
 *
 * A plain `===` returns at the first mismatched byte, so response time leaks
 * how many leading bytes of the guess were right. timingSafeEqual always
 * compares every byte. Length is checked first because timingSafeEqual throws
 * on mismatched lengths (same pattern as app/api/remove/route.ts).
 */
