...
def send_html_report(recipient: str, subject: str, html_file: Path) -> int:
    """Send a caller-provided HTML review email through the configured transport."""
    load_env()
    if not html_file.exists() or not html_file.is_file():
        print(f"HTML report file not found: {html_file}")
        return 2
    report_html = html_file.read_text(encoding="utf-8", errors="replace")
    user = os.environ.get("GMAIL_USER", "").strip()
    password = _gmail_app_password()
    resend_key = os.environ.get("RESEND_API_KEY", "").strip()
    if not resend_key and (not user or not password):
...
def main() -> int:
    ...
    parser.add_argument("command", choices=["env-status", "github-auth", "normalize", "validate", "test-email", "deploy", "verify-site", "send-report", "send-html-report", "send-status-report", "publish", "finalize"])
    ...
    parser.add_argument("--html-file", help="HTML file for send-html-report")
    ...
    if args.command == "send-html-report":
        if not args.recipient or not args.subject or not args.html_file:
            ...
            return 2
        return send_html_report(args.recipient, args.subject, Path(args.html_file).resolve())
