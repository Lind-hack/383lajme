...
export default function PortofoliPage() {
  ...
  return <div className="tregu-scope"><Navbar/><main className={styles.page}>
    <header className={styles.hero}><div><Link href="/tregu" className={styles.back}><span aria-hidden>&#8592;</span> Kthehu te Tregu</Link><h1>Portofoli yt</h1><p>Kapitali aktiv, rezultati i realizuar dhe çdo tregti e mbyllur — pa e numëruar një investim të hapur si humbje.</p></div><div className={styles.heroBalance}><span>Bilanci i realizuar</span><strong>{fmtNum(stats?.realizedBalance ?? profile?.coins ?? 0)} <small>383C</small></strong><b data-tone={tone(stats?.pnl30d)}>{signed(stats?.pnl30d ?? 0, 0)} në 30 ditë</b></div></header>
