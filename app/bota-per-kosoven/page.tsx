...
export const metadata: Metadata = {
  title: "Bota për Kosovën — artikujt dhe analiza",
  description:
    "Çfarë shkruajnë mediat e huaja për Kosovën: artikujt e përkthyer dhe analiza e tonit, në një vend — me fjalët e sakta që vendosin çdo vlerësim dhe me atë që është përjashtuar.",
};
...
export default async function BotaPerKosovenPage() {
  ...
  // Artikujt: the coverage itself. Same cache the analysis rests on, so the
  // two halves of this page can never describe different days.
  const foreignCoverage = getForeignCoverage(cache, 6);
  const coveragePool = Object.values(cache?.articles ?? {}).filter(
    (a) => a.imageUrl && a.translated
  );
  const coverageCountries = new Set(coveragePool.map((a) => a.country)).size;
  ...
  return (
    <>
      ...
      <main className="toni-page">
        ...
        {/* ── Artikujt ─────────────────────────────────────────────────────
            The coverage itself, before any analysis of it. A reader who only
            wants to know what the world published today stops here. */}
        <BotaFlet
          items={foreignCoverage}
          totalArticles={coveragePool.length}
          countryCount={coverageCountries}
        />

        {/* ── The map and the drill-down ───────────────────────────────────
            The same component the homepage runs, so the two cannot disagree
            about what a country's index is. */}
        <section className="toni-map-block" id="analiza">
