...
export default function TreguAdminClient() {
  ...
  const createHungaryF1Drafts = async () => {
    ...
    setDrafting(true);
    const created = await fetch("/api/admin/tregu/markets", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ question: "Kush fiton Çmimin e Madh të Hungarisë 2026?", description: "Një treg i vetëm me të gjithë pilotët; gridi zyrtar vendoset pas kualifikimit.", category: "Sport", closesInDays: 2, status: "draft", resolutionRules: "Fituesi është piloti P1 në klasifikimin zyrtar final të F1.", resolutionSource: "Formula 1 Dashboard / klasifikimi zyrtar F1" }) }).then(r => r.json());
    if (created.market?.id) await fetch(`/api/admin/tregu/markets/${created.market.id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ market_classification: "live_f1", market_type: "f1_race_winner", outcomes: drivers.map(([code]) => code), sport_outcomes: drivers.map(([code, label, team]) => ({ key: code, label, team })), outcome_quantities: Object.fromEntries(drivers.map(([code]) => [code, 0])), reference_probabilities: Object.fromEntries(drivers.map(([code]) => [code, 1 / drivers.length])), live_event: { provider: "formula1_dashboard", event_id: "hungarian-grand-prix-2026" } }) });
            ...
            {byStatus("draft").map((m) => (
              <div key={m.id} style={card}>
                ...
                <label style={{ display: "grid", gap: 5, width: "fit-content", marginTop: 12, fontSize: 12, fontWeight: 700 }}>
                  ...
                  <select
                    aria-label="Klasifikimi i tregut"
                    value={m.market_classification ?? "general_news"}
                    onChange={(event) => {
                      const value = event.target.value as MarketClassification;
                      marketAction(m.id, { market_classification: value });
                    }}
                ...
                {m.market_classification === "live_f1" && m.market_type !== "f1_race_winner" && <F1ConfigEditor market={m} onSaved={loadMarkets} />}
                <label style={{ display: "grid", gap: 5, width: "fit-content", marginTop: 12, fontSize: 12, fontWeight: 700 }}>
                  Lloji i tregut
                  <select
                  ...
                  >
                    ...
                    <option value="f1_race_winner">F1 Race Winner — 20–22 pilotë</option>
                  </select>
                </label>
  ...
  return (
    <div>
      <div style={{ display: "flex", gap: 8, alignItems: "center", marginBottom: 6 }}>
        ...
        <span style={{ fontSize: 10, fontWeight: 700, color: "#6B6B6B", background: "#F3F4F6", borderRadius: 999, padding: "2px 6px" }}>
          {m.market_type === "f1_race_winner" ? "F1 · 20–22 pilotë" : m.market_type === "three_outcome" ? "3 rezultate" : m.market_type === "two_outcome" ? "2 rezultate" : "PO/JO"}
        </span>
...
function F1ConfigEditor({ market, onSaved }: { market: Market; onSaved: () => void }) {
